...
async function writeWebIcons() {
  const targets = [
    path.join(forgeRoot, "favicon.png"),
  ...
  await writeIco(master.smallDark, path.join(forgeRoot, "favicon.ico"), [16, 32, 48, 64]);
  await writeIco(master.smallDark, path.join(forgeRoot, "public", "favicon.ico"), [16, 32, 48, 64]);
  await writeSvgFromPng(master.smallDark, path.join(forgeRoot, "favicon.svg"));
