...
Examples:
  ...
  FORGE_RELEASE_MODE=publish-from-tag ./scripts/release/release-forge-openclaw-plugin.sh 1.2.3
...
PY
    elif [[ "${file}" == "${HERMES_PLUGIN_PACKAGE_VERSION}" ]]; then
      python3 - "${version}" "${file}" <<'PY'
import re
import sys
from pathlib import Path
