...
test("the authoritative contract has the expected current and planned inventory", async () => {
  ...
  assert.equal(stories.length, 206);
  assert.equal(new Set(stories.map((story) => story.id)).size, 206);
  assert.equal(count("lifecycle", "current"), 206);
  assert.equal(count("lifecycle", "planned"), 0);
  assert.equal(count("readiness", "Verified"), 24);
  assert.equal(count("readiness", "In review"), 166);
  ...
  assert.deepEqual(
    ...
    CURRENT_READINESS_STATES.filter((state) => state !== "Limited").sort()
  );
...
test("the generated page is deterministic and contains every story exactly once", async () => {
  ...
  assert.equal(first.match(/class="story-card"/g)?.length, 206);
...
test("the client restores filters, canonicalizes contradictory URLs, and keeps matches visible", async () => {
  ...
  assert.equal(visibleStories(document).length, 24);
  assert.equal(
...
test("the client supports search, empty recovery, reset focus, and safe anchors", async () => {
  ...
  assert.equal(visibleStories(defaultDom.window.document).length, 166);
  ...
  assert.equal(visibleStories(defaultDom.window.document).length, 206);
  ...
  assert.equal(visibleStories(document).length, 206);

  const search = document.querySelector("#story-search");
  search.focus();
  search.value = "HealthKit";
  search.dispatchEvent(new dom.window.Event("input", { bubbles: true }));
  assert.equal(visibleStories(document).length, 4);
  assert.equal(
  ...
  assert.equal(visibleStories(document).length, 1);
