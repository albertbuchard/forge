...
test("the authoritative contract has the expected current and planned inventory", async () => {
  ...
  assert.equal(count("lifecycle", "current"), 189);
  assert.equal(count("lifecycle", "planned"), 14);
  assert.equal(count("readiness", "Verified"), 14);
  assert.equal(count("readiness", "In review"), 145);
...
test("the client supports search, empty recovery, reset focus, and safe anchors", async () => {
  ...
  assert.equal(visibleStories(defaultDom.window.document).length, 145);
  ...
  assert.equal(visibleStories(document).length, 189);
