...
import {
  ...
  KNOWLEDGE_GRAPH_RELATION_FAMILY_LABELS,
  KNOWLEDGE_GRAPH_RELATION_LABELS,
  buildKnowledgeGraphNodeId,
  type KnowledgeGraphEdge,
  type KnowledgeGraphEntityKind,
  type KnowledgeGraphFacets,
  type KnowledgeGraphFocusFamilyGroup,
...
export type KnowledgeGraphHierarchyNode = KnowledgeGraphNode & {
  layer: number;
  laneId: string;
  laneLabel: string;
  row: number;
...
export type KnowledgeGraphHierarchyEdge = KnowledgeGraphEdge & {
  secondary: boolean;
};

export type RenderedKnowledgeGraphEdge = {
  id: string;
  source: string;
  target: string;
  relationKind: KnowledgeGraphEdge["relationKind"];
  family: KnowledgeGraphEdge["family"];
  label: string;
  strength: number;
  directional: boolean;
  structural: boolean;
...
export function getKnowledgeGraphNodeVisual(
  node: Pick<KnowledgeGraphNode, "entityKind">
): EntityVisualDefinition {
  const kind = isEntityKind(node.entityKind) ? node.entityKind : "note";
  return getEntityVisual(kind);
}

export function getKnowledgeGraphNodeLayer(kind: KnowledgeGraphEntityKind) {
  const index = KNOWLEDGE_GRAPH_HIERARCHY_ORDER.indexOf(kind);
  return index >= 0 ? index : KNOWLEDGE_GRAPH_HIERARCHY_ORDER.length - 1;
}

export function getKnowledgeGraphNodeLane(kind: KnowledgeGraphEntityKind) {
  const lane =
    KNOWLEDGE_GRAPH_HIERARCHY_LANES.find((entry) =>
      entry.kinds.some((entryKind) => entryKind === kind)
    ) ??
    KNOWLEDGE_GRAPH_HIERARCHY_LANES[KNOWLEDGE_GRAPH_HIERARCHY_LANES.length - 1];
  return {
    laneId: lane.id,
    laneLabel: lane.label,
    laneIndex: KNOWLEDGE_GRAPH_HIERARCHY_LANES.findIndex(
...
function buildNeighborMap(edges: KnowledgeGraphEdge[]) {
  const neighborMap = new Map<string, Set<string>>();
  for (const edge of edges) {
    const sourcePeers = neighborMap.get(edge.source) ?? new Set<string>();
    sourcePeers.add(edge.target);
    neighborMap.set(edge.source, sourcePeers);
    const targetPeers = neighborMap.get(edge.target) ?? new Set<string>();
...
function getNodeUpdatedAtValue(node: KnowledgeGraphNode) {
  if (!node.updatedAt) {
    return null;
  }
...
export function compareKnowledgeGraphNodes(
  left: KnowledgeGraphNode,
  right: KnowledgeGraphNode
) {
  return (
...
function compareKnowledgeGraphEdges(
...
export function buildRenderedKnowledgeGraphEdges(edges: KnowledgeGraphEdge[]) {
  const edgesByPair = new Map<string, KnowledgeGraphEdge[]>();
  ...
  return Array.from(edgesByPair.entries())
    .map(([pairKey, pairEdges]) => {
      ...
      const strength =
        Math.max(...sortedEdges.map((edge) => edge.strength)) *
      ...
      return {
        ...
        parallelCount,
        data: sortedEdges
      } satisfies RenderedKnowledgeGraphEdge;
    })
    .sort(
      (left, right) =>
...
export function buildKnowledgeGraphDatasetSignature(
...
export function filterKnowledgeGraphData(
  graph: Pick<KnowledgeGraphPayload, "nodes" | "edges">,
...
export function selectKnowledgeGraphVisibleNodeIds({
  nodes,
  edges,
  limit,
  focusNodeId
...
export function buildKnowledgeGraphFacets(
  nodes: KnowledgeGraphNode[],
  edges: KnowledgeGraphEdge[]
): KnowledgeGraphFacets {
  const entityKinds = nodes.reduce<Record<string, number>>((counts, node) => {
  ...
  const tags = new Map<string, { id: string; label: string; count: number }>();
  ...
  >();
  let minUpdatedAt: string | null = null;
  ...
  return {
    entityKinds: Object.entries(entityKinds)
      .map(([value, count]) => ({
        value: value as KnowledgeGraphEntityKind,
    ...
    relationKinds: Object.entries(relationKinds)
      .map(([value, count]) => ({
        ...
        label:
          KNOWLEDGE_GRAPH_RELATION_LABELS[
            value as KnowledgeGraphEdge["relationKind"]
          ],
        count
      }))
      .sort((left, right) => right.count - left.count || left.label.localeCompare(right.label)),
...
export function buildKnowledgeGraphHierarchy(
  nodes: KnowledgeGraphNode[],
  edges: KnowledgeGraphEdge[]
): {
  nodes: KnowledgeGraphHierarchyNode[];
  edges: KnowledgeGraphHierarchyEdge[];
} {
  const sortedNodes = [...nodes].sort((left, right) => {
    const layerDelta =
      getKnowledgeGraphNodeLayer(left.entityKind) -
  ...
  const hierarchyNodes = sortedNodes.map((node) => {
    const { laneId, laneIndex, laneLabel } = getKnowledgeGraphNodeLane(
      node.entityKind
    );
...
export function buildKnowledgeGraphFocusPayload(
  nodes: KnowledgeGraphNode[],
  edges: KnowledgeGraphEdge[],
  ...
  for (const relation of groupsByRelation.values()) {
    ...
    if (existing) {
      existing.relations.push({
        ...relation,
        items: [...relation.items].sort((left, right) =>
          left.title.localeCompare(right.title)
        )
      });
  ...
  return {
    generatedAt: new Date().toISOString(),
    focusNode,
    firstRingNodes: firstRingNodes.sort((left, right) =>
      left.title.localeCompare(right.title)
    ),
    neighborhoodEdges,
...
export function getKnowledgeGraphNodeDegree(
  nodeId: string,
  edges: KnowledgeGraphEdge[]
) {
...
export function isKnowledgeGraphNodeFocused(
  nodeId: string,
  focusNodeId: string | null,
  edges: KnowledgeGraphEdge[]
) {
...
export function buildKnowledgeGraphFocusNodeId(
...
export function getKnowledgeGraphFocusRelatedNodeIds(
  focusNodeId: string | null,
  edges: KnowledgeGraphEdge[]
) {
  if (!focusNodeId) {
