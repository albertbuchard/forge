...
export type GamificationAtomicRequirement = {
...
export type GamificationAssetManifestEntry = {
  key: string;
  role: "item" | "mascot" | "streak" | "skin";
  itemKind?: GamificationCatalogKind;
  alt: string;
  dominantColor: string;
  tierFrame: GamificationCatalogTier | "none";
  sourcePath: string;
  spritePath: string;
...
type CategoryConfig = {
  id: GamificationCatalogCategory;
  label: string;
  trophyCount: number;
  unlockCount: number;
};
...
function metric(metric: GamificationMetricKey, threshold: number): GamificationRequirement {
  return { metric, threshold };
}

function allOf(...requirements: GamificationRequirement[]): GamificationRequirement {
...
function tierForIndex(index: number, total: number): GamificationCatalogTier {
  const ratio = (index + 1) / total;
  if (ratio > 0.82) return "platinum";
  if (ratio > 0.55) return "gold";
  if (ratio > 0.25) return "silver";
  return "bronze";
}

function difficultyForTier(tier: GamificationCatalogTier): GamificationDifficulty {
  if (tier === "platinum") return "legendary";
  if (tier === "gold") return "hard";
  if (tier === "silver") return "standard";
  return "intro";
}

function rarityForTier(tier: GamificationCatalogTier): GamificationCatalogRarity {
  if (tier === "platinum") return "legendary";
...
function itemAssetKey(seed: CatalogSeed) {
...
function unlock(
  category: GamificationCatalogCategory,
  title: string,
  unlockType: GamificationUnlockType,
  requirement: GamificationRequirement,
  requirementText: string,
  summary: string,
  rewardPayload: Record<string, string | number | boolean | null>
...
function buildCatalog(): GamificationCatalogItem[] {
  let sortOrder = 0;
  let trophySheetIndex = 0;
  let unlockSheetIndex = 0;
  const categorySeen = new Map<GamificationCatalogCategory, { trophy: number; unlock: number }>();
  return SEEDS.map((seed) => {
    ...
    const categoryKindTotal =
      seed.kind === "trophy" ? config.trophyCount : config.unlockCount;
    ...
    return {
      ...
      sheetKey: `${seed.kind === "trophy" ? "trophies" : "unlocks"}-r${
        Math.floor(sheetIndex / 10) + 1
      }-c${(sheetIndex % 10) + 1}`,
...
function assertCatalog() {
  if (GAMIFICATION_CATALOG.length !== 144) {
    throw new Error(
      `GAMIFICATION_CATALOG must contain exactly 144 items; found ${GAMIFICATION_CATALOG.length}.`
    );
  }
  ...
  if (trophyCount !== 96 || unlockCount !== 48) {
    throw new Error(
      `GAMIFICATION_CATALOG must contain 96 trophies and 48 unlocks; found ${trophyCount}/${unlockCount}.`
    );
  }
  for (const config of CATEGORY_CONFIGS) {
    const trophies = GAMIFICATION_CATALOG.filter(
      (item) => item.category === config.id && item.kind === "trophy"
    ).length;
    const unlocks = GAMIFICATION_CATALOG.filter(
      (item) => item.category === config.id && item.kind === "unlock"
    ).length;
    if (trophies !== config.trophyCount || unlocks !== config.unlockCount) {
      throw new Error(
        `${config.id} expected ${config.trophyCount}/${config.unlockCount}, found ${trophies}/${unlocks}.`
      );
    }
  }
...
export const GAMIFICATION_MASCOT_STATE_KEYS = Array.from(
  { length: 30 },
  (_, index) => `mascot-state-${String(index + 1).padStart(3, "0")}`
);
...
if (GAMIFICATION_MASCOT_KEYS.length !== 30) {
  throw new Error(
    `GAMIFICATION_MASCOT_KEYS must contain exactly 30 mascot/state keys; found ${GAMIFICATION_MASCOT_KEYS.length}.`
  );
}

function manifestColor(item: GamificationCatalogItem) {
...
function assetPromptForItem(item: GamificationCatalogItem) {
  return [
...
const itemManifestEntries = Object.fromEntries(
  GAMIFICATION_CATALOG.map((item) => [
    item.assetKey,
    {
      key: item.assetKey,
      role: "item",
    ...
    } satisfies GamificationAssetManifestEntry
  ])
);

const mascotManifestEntries = Object.fromEntries(
  GAMIFICATION_MASCOT_KEYS.map((key) => {
    ...
    return [
      key,
      {
        ...
        sheetKey: null,
        prompt:
          "dramatic Forge Smith blacksmith mascot, premium game art, no text, no watermark"
      } satisfies GamificationAssetManifestEntry
    ];
  })
);

export const GAMIFICATION_ASSET_MANIFEST: Record<
  string,
  GamificationAssetManifestEntry
> = {
  ...itemManifestEntries,
