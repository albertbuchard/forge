...
export function ProjectsPage() {
  ...
  }, [shell.snapshot.dashboard.goals, shell.snapshot.dashboard.projects, shell.snapshot.tags, shell.snapshot.tasks]);
  ...
  const searchOptions = useMemo(() => {
    ...
    for (const goal of shell.snapshot.dashboard.goals) {
      ...
      optionMap.set(`goal:${goal.id}`, {
        ...
        value: goal.id,
        label: goal.title,
        description: `${projectCount} project${projectCount === 1 ? "" : "s"} linked to this goal`,
    ...
    for (const task of shell.snapshot.tasks) {
      ...
      optionMap.set(`task:${task.id}`, {
        ...
        value: task.id,
        label: task.title,
        description: `${projectCount} project${projectCount === 1 ? "" : "s"} linked to this task`,
    ...
    for (const tag of shell.snapshot.tags) {
      ...
      optionMap.set(`tag:${tag.id}`, {
        ...
        value: tag.id,
        label: tag.name,
        description: `${projectCount} project${projectCount === 1 ? "" : "s"} touching this tag`,
  ...
  }, [collectionCounts, projectFacets, shell.snapshot.dashboard.goals, shell.snapshot.dashboard.projects, shell.snapshot.tags, shell.snapshot.tasks]);
