...
export function PsycheBehaviorsPage() {
  ...
  const steps: Array<QuestionFlowStep<BehaviorInput>> = [
      ...
      render: (value, setValue) => (
        <>
          ...
          <FlowField label="What does the move look like?">
            <Textarea value={value.description} onChange={(event) => setValue({ description: event.target.value })} placeholder="Describe the move the way you would recognize it in the moment, not in therapist shorthand." />
          </FlowField>
        </>
      )
    },
    {
      ...
      description: "Make the move readable as a real pattern instead of a label.",
      render: (value, setValue) => (
        <>
          ...
          <FlowField label="What inner push or story shows up?">
            <Textarea value={value.urgeStory} onChange={(event) => setValue({ urgeStory: event.target.value })} placeholder="What inner push or justification tends to appear?" />
          </FlowField>
          <FlowField label="What do you get right away from this move?">
            <Textarea value={value.shortTermPayoff} onChange={(event) => setValue({ shortTermPayoff: event.target.value })} placeholder="What relief, certainty, distance, or control does it give in the short term?" />
          </FlowField>
        </>
      )
    },
    {
      ...
      description: "Once the move is clear, decide whether it is an away move, a committed action, or a recovery path.",
      render: (value, setValue) => (
        <>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <FlowField label="What does it cost over time?">
              <Textarea value={value.longTermCost} onChange={(event) => setValue({ longTermCost: event.target.value })} placeholder="What does this move cost over time?" />
            </FlowField>
            <FlowField label="What move should replace or steady it?">
              <Textarea value={value.replacementMove} onChange={(event) => setValue({ replacementMove: event.target.value })} placeholder="What move should replace this one when possible?" />
            </FlowField>
          </div>
          <FlowField label="If the slip happens, how do you repair and return?">
            <Textarea value={value.repairPlan} onChange={(event) => setValue({ repairPlan: event.target.value })} placeholder="Describe the repair path without shame or collapse." />
          </FlowField>
        </>
      )
    },
    {
      ...
      description: "This turns the move into part of the full graphical psyche system.",
      render: (value, setValue) => (
        <>
  ...
  if (behaviorsQuery.isLoading || patternsQuery.isLoading || valuesQuery.isLoading || schemasQuery.isLoading || modesQuery.isLoading) {
    return <LoadingState eyebrow="Behaviors" title="Loading behaviors" description="Getting behaviors, patterns, values, schemas, and modes ready." />;
  }

  const routeError = behaviorsQuery.error ?? patternsQuery.error ?? valuesQuery.error ?? schemasQuery.error ?? modesQuery.error;
  if (routeError) {
    return <ErrorState eyebrow="Psyche behaviors" error={routeError} onRetry={() => void Promise.all([behaviorsQuery.refetch(), patternsQuery.refetch(), valuesQuery.refetch(), schemasQuery.refetch(), modesQuery.refetch()])} />;
  }
  ...
  return (
    <div className="grid gap-5">
      <PageHero
        entityKind="behavior"
        title={<EntityName kind="behavior" label="Behaviors" variant="heading" size="lg" />}
          ...
          entries={[
            {
              ...
              summary: grouped.away[0]?.replacementMove || "Map the move that tends to pull you away first.",
              href: "#behavior-columns",
              tone: "away"
            },
            {
              id: "committed",
              title: grouped.committed[0]?.title ?? "No committed action mapped yet",
              summary: grouped.committed[0]?.replacementMove || "Map the move you want to practice instead.",
              href: "#behavior-columns",
              tone: "committed"
            },
            {
              id: "recovery",
              title: grouped.recovery[0]?.title ?? "No recovery move mapped yet",
              summary: grouped.recovery[0]?.repairPlan || "Map how you return after a slip without turning it into collapse.",
              href: "#behavior-columns",
              tone: "recovery"
            }
          ]}
      ...
      >
        <div id="behavior-columns" className="grid gap-4 xl:grid-cols-3">
          {(["away", "committed", "recovery"] as const).map((kind) => (
            <div key={kind} className="grid gap-3 rounded-[24px] bg-white/[0.04] p-4">
              <div className="flex items-center justify-between gap-3">
                <div className="font-medium text-white">{kindTitleMap[kind]}</div>
      ...
      <QuestionFlowDialog
        ...
        onSubmit={async () => {
          ...
          if (!parsed.success) {
            setSubmitError("This behavior still needs a kind and a title before it can be saved.");
            return;
          }
          ...
          try {
            ...
            setSubmitError(error instanceof Error ? error.message : "Unable to save this behavior right now.");
          }
        }}
      />
    </div>
