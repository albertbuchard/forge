...
export function GoalsPage() {
  ...
  return (
    <div className="grid gap-5">
      <PageHero
        entityKind="goal"
        title={<EntityName kind="goal" label="Life Goals" variant="heading" size="lg" />}
        ...
        actions={
          hasGoals ? (
            <div className="inline-flex min-h-10 items-center rounded-full border border-white/8 bg-white/[0.04] px-4 py-2 text-sm whitespace-nowrap text-white/68">
              {shell.snapshot.dashboard.projects.length} live project{shell.snapshot.dashboard.projects.length === 1 ? "" : "s"} attached
            </div>
          ) : null
        }
      />
      ...
      {hasGoals ? (
                  ...
                  <div className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] xl:gap-5">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge className="bg-white/[0.08] text-white/72">{goal.horizon}</Badge>
                      </div>
                      <div className="mt-4">
                        <EntityName kind="goal" label={goal.title} variant="heading" size="xl" />
                      </div>
                      <p className="mt-3 max-w-2xl text-sm leading-7 text-white/58">{goal.description}</p>
                      <div className="mt-4 grid gap-3 sm:grid-cols-3">
                        <div className="rounded-[18px] bg-white/[0.03] px-3.5 py-3">
                          <div className="font-label text-[11px] uppercase tracking-[0.16em] text-white/40">Progress</div>
                          <div className="mt-2 text-xl text-white">{goal.progress}%</div>
                        </div>
                        <div className="rounded-[18px] bg-white/[0.03] px-3.5 py-3">
                          <div className="font-label text-[11px] uppercase tracking-[0.16em] text-white/40">Tasks done</div>
                          <div className="mt-2 text-xl text-white">{goal.completedTasks}/{Math.max(goal.totalTasks, 1)}</div>
                        </div>
                        <div className="rounded-[18px] bg-white/[0.03] px-3.5 py-3">
                          <div className="font-label text-[11px] uppercase tracking-[0.16em] text-white/40">XP banked</div>
                          <div className="mt-2 text-xl text-white">{goal.earnedPoints}</div>
                        </div>
                      </div>
                      <div className="mt-4">
                    ...
                    <div className="rounded-[24px] bg-white/[0.04] p-4">
                      <div className="flex items-center justify-between gap-3">
                        <div className="font-medium text-white">Projects carrying this direction</div>
                      ...
                      {linkedProjects.length === 0 ? (
                        ...
                        <div className="mt-4 grid gap-3">
                          {linkedProjects.slice(0, 3).map((project) => (
                            <div key={project.id} className="rounded-[18px] bg-white/[0.05] px-4 py-4">
                              <div className="flex items-center justify-between gap-3">
                              ...
                              <div className="mt-2 text-sm leading-6 text-white/58">{project.description}</div>
        ...
        <Card>
          <div className="rounded-[20px] bg-white/[0.04] p-4 text-sm text-white/58">Goal cards will appear here once the first long-horizon direction is created.</div>
        </Card>
      )}
    </div>
  );
