...
export function PsycheValuesPage() {
  ...
  if (valuesQuery.isLoading) {
    return (
      <LoadingState
        eyebrow="Psyche values"
        title="Loading value constellation"
        description="Hydrating values, linked goals, projects, and tasks."
      />
    );
  }
  ...
  return (
    <div className="grid gap-5">
      <PageHero
        entityKind="value"
        title={
          <EntityName kind="value" label="Values" variant="heading" size="lg" />
        }
      ...
      >
        <div id="values-atlas" className="grid gap-4">
          {values.length === 0 ? (
            ...
            values.map((value) => {
              const linkedGoals = shell.snapshot.goals.filter((goal) =>
                value.linkedGoalIds.includes(goal.id)
              );
              const linkedProjects = shell.snapshot.dashboard.projects.filter(
                (project) => value.linkedProjectIds.includes(project.id)
              );
              const linkedTasks = shell.snapshot.tasks.filter((task) =>
                value.linkedTaskIds.includes(task.id)
              );
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {value.user ? (
                        <UserBadge user={value.user} compact />
                      ) : null}
                      <EntityNoteCountLink
          ...
          try {
            ...
            setSubmitError(
              error instanceof Error
                ? error.message
                : "Unable to save this value right now."
            );
          }
        }}
      />
