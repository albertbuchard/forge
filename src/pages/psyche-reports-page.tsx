  ...
  const steps: Array<QuestionFlowStep<ReportDraft>> = [
    {
      ...
      description:
        "Start with what happened concretely before interpretation takes over.",
        ...
        <>
          ...
          <FlowField label="Situation">
            <Textarea
              ...
              placeholder="Describe what happened concretely, without explaining it away yet."
            />
          </FlowField>
        </>
      )
    },
    {
      ...
      description:
        "These stay structured, but the flow keeps them staged instead of dumping everything into one giant form.",
      render: (value, setValue) => (
        <>
          <EmotionRowsEditor
    ...
    },
    {
      ...
      description:
        "This is where the report becomes part of the larger graphical psyche system.",
      render: (value, setValue) => (
        <>
  ...
  if (
    ...
    return (
      <LoadingState
        ...
        description="Getting reports, values, behaviors, beliefs, modes, event labels, and emotions ready."
      />
    );
  }
  ...
  return (
    <div className="grid gap-5">
      ...
      <QuestionFlowDialog
        ...
        value={draft}
        onChange={setDraft}
        draftPersistenceKey="psyche.report.new"
        steps={steps}
        ...
        onSubmit={async () => {
          ...
          const parsed = triggerReportSchema.safeParse({
            ...
            occurredAt: draft.occurredAt
              ...
              : null,
            emotions: draft.emotions.filter(
              (entry) => entry.label.trim().length > 0
            ),
            thoughts: draft.thoughts.filter(
              (entry) => entry.text.trim().length > 0
            ),
            behaviors: draft.behaviors.filter(
              (entry) => entry.text.trim().length > 0
            ),
            consequences: {
            ...
            modeTimeline: draft.modeTimeline.filter(
              (entry) =>
                entry.stage.trim().length > 0 && entry.label.trim().length > 0
            ),
          ...
          if (!parsed.success) {
            setSubmitError(
              "This report still needs a title and concrete triggering situation."
            );
            return;
          }
