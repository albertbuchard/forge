...
function RouteSubviewPulse({ index }: { index: number }) {
  return (
    <div
      className="surface-pulse min-h-[7.5rem] rounded-[22px] border border-[var(--ui-border-subtle)] bg-[var(--ui-surface-1)] p-4"
      style={{ animationDelay: `${index * 90}ms` }}
    >
      <div className="h-2.5 w-24 rounded-full bg-[var(--ui-surface-3)]" />
      <div className="mt-4 h-9 w-full rounded-2xl bg-[var(--ui-surface-2)]" />
      <div className="mt-3 grid grid-cols-3 gap-2">
        <div className="h-3 rounded-full bg-[var(--ui-surface-2)]" />
        <div className="h-3 rounded-full bg-[var(--ui-surface-1)]" />
        <div className="h-3 rounded-full bg-[var(--ui-surface-1)]" />
      </div>
    </div>
  );
...
export function RouteViewLoadingSurface({ meta }: { meta: RouteViewMeta }) {
  ...
  return (
    <motion.section
      key={meta.surfaceId}
      className={cn(
        "route-view-loading relative isolate grid min-h-[28rem] gap-5 overflow-hidden rounded-[28px] border border-[var(--ui-border-subtle)] p-4 shadow-[var(--ui-shadow-soft)] sm:p-5",
    ...
    >
      <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-[linear-gradient(90deg,transparent,var(--ui-border-strong),transparent)]" />
      <div className="pointer-events-none absolute inset-x-6 top-0 h-24 bg-[linear-gradient(180deg,var(--ui-surface-2),transparent)] opacity-70" />

      <header className="relative flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <div className="font-label text-[10px] uppercase tracking-[0.16em] text-[var(--ui-ink-faint)]">
            Opening surface
          </div>
          <h1 className="mt-2 font-display text-2xl font-semibold tracking-normal text-[var(--ui-ink-strong)] sm:text-3xl">
            {meta.title}
          </h1>
        ...
        <div className="inline-flex w-fit items-center gap-2 rounded-full border border-[var(--ui-border-subtle)] bg-[var(--ui-surface-2)] px-3 py-2 text-[12px] font-medium text-[var(--ui-ink-soft)]">
          <span className="relative flex size-2.5">
            <span className="absolute inline-flex size-full animate-ping rounded-full bg-[var(--primary)]/30" />
