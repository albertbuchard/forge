...
function TargetTable({
  ...
  return (
    <div className="overflow-hidden rounded-[20px] border border-[var(--ui-border-subtle)]">
        ...
        <table className="w-full min-w-[620px] border-collapse text-left text-sm">
          <thead className="sticky top-0 bg-[var(--ui-surface-2)] text-xs uppercase text-[var(--ui-ink-faint)]">
            <tr>
              ...
              {!compact ? <th className="px-4 py-3 font-semibold">Source</th> : null}
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--ui-border-subtle)]">
            {rows.map((target) => (
              <tr key={target.id} className="bg-[var(--ui-surface-1)]">
                <td className="px-4 py-3 font-medium text-[var(--ui-ink)]">{target.label}</td>
                <td className="px-4 py-3 text-[var(--ui-ink)]">
                  <span className="font-semibold">{target.target}</span>{" "}
                  <span className="text-[var(--ui-ink-faint)]">{target.unit}</span>
                </td>
                <td className="px-4 py-3 text-[var(--ui-ink-muted)]">{target.note}</td>
                {!compact ? (
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm text-[var(--ui-ink-muted)]">
              Daily targets use Forge plan values first, then adult DRI/Dietary Guidelines
              defaults for nutrients the food log can score as catalog detail improves.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Badge tone="signal">{targets.profile.calorieTarget.toFixed(0)} kcal</Badge>
            <Badge tone="meta">
          ...
          <div className="grid content-start gap-4">
            ...
            >
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                />
                <SurfaceStat
                  label="Sport equivalent"
                  value={sportEquivalent}
                />
                <SurfaceStat label="Sweat fluid" value={`${targets.sportSummary.fluidLossLiters} L`} />
                <SurfaceStat label="Sodium loss" value={`${targets.sportSummary.sodiumLossMg} mg`} />
              </div>
              <div className="mt-4 flex items-start gap-3 rounded-[18px] border border-[var(--ui-border-subtle)] bg-[var(--ui-surface-1)] p-3 text-sm text-[var(--ui-ink-muted)]">
                ...
                <span>
                  Sport burn is shown as gross energy equivalent, not a promise of scale
                  loss. The calorie plan may already include active burn, and replacement
                  depends on appetite, heat, sodium loss, and actual intake.
                </span>
              </div>
            </TargetSection>
                ...
                {targets.sportLosses.map((target) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-xs text-[var(--ui-ink-faint)]">{target.note}</p>
                    </div>
