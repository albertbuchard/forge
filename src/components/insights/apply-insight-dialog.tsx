...
import { goalMutationSchema, projectMutationSchema, quickTaskSchema, type GoalMutationInput, type ProjectMutationInput, type QuickTaskInput } from "@/lib/schemas";
...
function buildInitialDraft(insight: Insight, goals: Goal[], projects: ProjectSummary[], tasks: Task[]): ApplyInsightDraft {
  return {
...
function getApplyChoiceCopy(kind: ApplyInsightKind) {
  switch (kind) {
    ...
    case "project":
      return {
        label: "Project",
        description: "Start a larger initiative when the insight needs a stream of work."
      };
    case "goal":
      return {
        label: "Goal",
        description: "Capture a new long-term direction when this changes strategy."
      };
    case "note":
      return {
        label: "Linked note",
        description: "Attach the recommendation as durable evidence without creating new work."
      };
  }
}
...
export function ApplyInsightDialog({
  ...
  const availableKinds = useMemo(() => getAvailableApplyKinds(insight, goals, projects), [goals, insight, projects]);
  const sourceLink = useMemo(() => getInsightSourceLink(insight), [insight]);
  const [draft, setDraft] = useState<ApplyInsightDraft>(() => buildInitialDraft(insight, goals, projects, tasks));
  ...
  const selectedProject = draft.task.projectId ? projects.find((project) => project.id === draft.task.projectId) ?? null : null;
  ...
  const selectedGoal = draft.project.goalId ? goals.find((goal) => goal.id === draft.project.goalId) ?? null : null;
  const linkedTargetLabel = useMemo(() => {
    ...
    switch (sourceLink.entityType) {
      case "goal":
        return goals.find((goal) => goal.id === sourceLink.entityId)?.title ?? "Linked goal";
      case "project":
        return projects.find((project) => project.id === sourceLink.entityId)?.title ?? "Linked project";
      case "task":
        return tasks.find((task) => task.id === sourceLink.entityId)?.title ?? "Linked task";
      default:
  ...
  const steps: Array<QuestionFlowStep<ApplyInsightDraft>> = [
    {
      ...
      description: "Accept means you agree with the recommendation. Apply means you turn it into a real record right now.",
      render: (value, patch) => (
        <FlowField
          label="Apply as"
    ...
    },
    {
      ...
      description: "The fields are prefilled from the insight, but you can tighten them before saving.",
      render: (value, patch) => {
        if (value.kind === "task") {
          return (
            <>
              <FlowField label="Task title" error={fieldErrors.title ?? null}>
                <Input value={value.task.title} onChange={(event) => patch({ task: { ...value.task, title: event.target.value } })} placeholder="Turn the recommendation into a task" />
              </FlowField>
              <FlowField label="Description">
                <Textarea value={value.task.description} onChange={(event) => patch({ task: { ...value.task, description: event.target.value } })} placeholder="Add the operational context for this task." />
              </FlowField>
                  ...
                  {projects.map((project) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-2 text-sm leading-6 text-white/54">{project.description || "No project note attached yet."}</div>
                      </button>
                    );
                  })}
                </div>
              ...
              <FlowField label="Owner" error={fieldErrors.owner ?? null}>
                <Input value={value.task.owner} onChange={(event) => patch({ task: { ...value.task, owner: event.target.value } })} placeholder="Albert" />
              </FlowField>
                      ...
                      <button
                        ...
                        onClick={() =>
                          patch({
                            task: {
                              ...value.task,
                              tagIds: selected ? value.task.tagIds.filter((entry) => entry !== tag.id) : [...value.task.tagIds, tag.id]
                            }
                          })
                        }
                      >
        ...
        if (value.kind === "project") {
          return (
            <>
              <FlowField label="Project title" error={fieldErrors.title ?? null}>
                <Input value={value.project.title} onChange={(event) => patch({ project: { ...value.project, title: event.target.value } })} placeholder="Turn the insight into a project" />
              </FlowField>
              <FlowField label="Description">
                <Textarea value={value.project.description} onChange={(event) => patch({ project: { ...value.project, description: event.target.value } })} placeholder="Describe the initiative this insight points to." />
              </FlowField>
                  ...
                  {goals.map((goal) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        className={`rounded-[22px] border px-4 py-4 text-left transition ${selected ? "border-[rgba(192,193,255,0.28)] bg-[rgba(192,193,255,0.14)] text-white" : "border-white/8 bg-white/[0.04] text-white/72 hover:bg-white/[0.07]"}`}
                        onClick={() => patch({ project: { ...value.project, goalId: goal.id } })}
                      >
              ...
              {selectedGoal ? <FieldHint>This project will sit under the “{selectedGoal.title}” goal.</FieldHint> : null}
            </>
          );
        }
          ...
          return (
            <>
              <FlowField label="Goal title" error={fieldErrors.title ?? null}>
                <Input value={value.goal.title} onChange={(event) => patch({ goal: { ...value.goal, title: event.target.value } })} placeholder="Turn the insight into a life goal" />
              </FlowField>
              <FlowField label="Description">
                <Textarea value={value.goal.description} onChange={(event) => patch({ goal: { ...value.goal, description: event.target.value } })} placeholder="Describe the strategic direction this insight suggests." />
              </FlowField>
              <FlowField label="Tags">
                <div className="flex flex-wrap gap-2">
                  {tags.map((tag) => {
                      ...
                      <button
                        ...
                        onClick={() =>
                          patch({
                            goal: {
                              ...value.goal,
                              tagIds: selected ? value.goal.tagIds.filter((entry) => entry !== tag.id) : [...value.goal.tagIds, tag.id]
                            }
                          })
                        }
                      >
        ...
        return (
          <>
            <FlowField
              ...
              hint={linkedTargetLabel ? `This note will attach to ${linkedTargetLabel}.` : undefined}
            >
              <Textarea
                value={value.noteMarkdown}
                onChange={(event) => patch({ noteMarkdown: event.target.value })}
            ...
            {linkedTargetLabel ? (
              <div className="rounded-[18px] border border-white/8 bg-white/[0.04] px-4 py-3 text-sm text-white/70">
                Linked target: <span className="font-medium text-white">{linkedTargetLabel}</span>
              </div>
            ) : null}
          </>
        );
        ...
        try {
          if (draft.kind === "task") {
            ...
            if (!parsed.success) {
              ...
              setSubmitError("This applied task still needs a valid title, project, and owner.");
              return;
            }
          ...
          if (draft.kind === "project") {
            ...
            if (!parsed.success) {
              setFieldErrors(Object.fromEntries(Object.entries(parsed.error.flatten().fieldErrors).map(([key, value]) => [key, value?.[0]])));
              setSubmitError("This applied project still needs a valid title and goal.");
              return;
            }
          ...
          if (draft.kind === "goal") {
            ...
            if (!parsed.success) {
              setFieldErrors(Object.fromEntries(Object.entries(parsed.error.flatten().fieldErrors).map(([key, value]) => [key, value?.[0]])));
              setSubmitError("This applied goal still needs a valid title and target.");
              return;
            }
          ...
          if (!draft.noteMarkdown.trim()) {
            setSubmitError("The linked note needs Markdown content before it can be saved.");
            return;
          }

          if (!sourceLink) {
            setSubmitError("This insight is not attached to a specific entity yet, so Forge cannot place a linked note for it.");
            return;
          }
          ...
          setSubmitError(error instanceof Error ? error.message : "Unable to apply this insight right now.");
        }
      }}
    />
  );
