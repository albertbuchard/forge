...
type InsightTargetKind = "general" | "goal" | "project" | "task" | "trigger_report";
...
function buildInitialValue(
  ...
  return {
    ...
    targetKind: lockedEntity?.entityType ?? (initialValue?.entityType as InsightTargetKind | undefined) ?? "general"
  };
}
...
export function InsightFlowDialog({
  ...
  const [value, setValue] = useState<InsightFlowValue>(() => buildInitialValue(initialValue, lockedEntity));
  ...
  const visibleCandidates = entityCandidates.filter((candidate) => candidate.entityType === value.targetKind);
  ...
  const steps: Array<QuestionFlowStep<InsightFlowValue>> = [
    {
      ...
      description: "Start with the main observation, then decide whether this belongs to a specific entity or should stay general.",
      render: (draft, patch) => (
        <>
          <FlowField
            label="Insight title"
          ...
          >
            <Input value={draft.title} onChange={(event) => patch({ title: event.target.value })} placeholder="Weekly review needs a tighter end ritual" />
          </FlowField>
          <FlowField label="Timeframe" description="Keep the horizon short and readable.">
            <Input value={draft.timeframeLabel} onChange={(event) => patch({ timeframeLabel: event.target.value })} placeholder="This week" />
          </FlowField>
          <FlowField
          ...
          >
            {lockedEntity ? (
              <div className="rounded-[24px] border border-white/8 bg-white/[0.04] p-4">
                <EntityBadge kind={lockedEntity.kind} label={lockedEntity.label} />
                {lockedEntity.description ? <FieldHint className="mt-2">{lockedEntity.description}</FieldHint> : null}
              </div>
            ) : (
              <>
                <FlowChoiceGrid
                  ...
                  visibleCandidates.length > 0 ? (
                    <div className="mt-4 grid gap-3 md:grid-cols-2">
                      {visibleCandidates.map((candidate) => {
                        const selected = draft.entityType === candidate.entityType && draft.entityId === candidate.entityId;
                        return (
                          <button
                            ...
                            onClick={() => patch({ entityType: candidate.entityType, entityId: candidate.entityId })}
                          >
                            <EntityBadge kind={candidate.kind} label={candidate.label} compact />
                            {candidate.description ? <div className="mt-2 text-sm leading-6 text-white/54">{candidate.description}</div> : null}
                          </button>
                        );
                      })}
                    </div>
                  ) : (
                    <FieldHint className="mt-4">No {draft.targetKind === "trigger_report" ? "reports" : `${draft.targetKind}s`} are ready to attach here yet.</FieldHint>
                  )
                ) : null}
              </>
            )}
    ...
    },
    {
      ...
      description: "Keep the summary plain and useful. Add the why only if it helps the next action make sense.",
      render: (draft, patch) => (
        <>
          <FlowField
            label="Summary"
          ...
          >
            <Textarea value={draft.summary} onChange={(event) => patch({ summary: event.target.value })} placeholder="The weekly review loses energy because the close-out is vague, so finished work never turns into clear next moves." />
          </FlowField>
          <FlowField label="Why this matters" description="Optional context that helps explain the recommendation.">
            <Textarea value={draft.rationale} onChange={(event) => patch({ rationale: event.target.value })} placeholder="When the close-out is loose, important wins disappear from memory and the next week starts without momentum." />
          </FlowField>
        </>
      )
    },
    {
      ...
      description: "Finish with the recommendation you want the user or agent to actually act on.",
      render: (draft, patch) => (
        <>
          <FlowField
          ...
          >
            <Textarea value={draft.recommendation} onChange={(event) => patch({ recommendation: event.target.value })} placeholder="Add a fixed five-minute close-out block that turns finished tasks into one sentence of evidence and one next move." />
          </FlowField>
          <FlowField label="Confidence" description="Choose how strongly this recommendation holds up right now.">
            <FlowChoiceGrid
              value={String(draft.confidence)}
              onChange={(next) => patch({ confidence: Number(next) })}
              options={[
                { value: "0.55", label: "Low", description: "Useful hunch, still early." },
                { value: "0.72", label: "Medium", description: "Solid pattern, worth acting on." },
                { value: "0.88", label: "High", description: "Clear enough to trust strongly." }
              ]}
              columns={3}
            />
          </FlowField>
  ...
  return (
    <QuestionFlowDialog
      ...
      onSubmit={async () => {
        ...
        if (!payload.success) {
          setSubmitError("This insight still needs a title, a summary, and a recommendation before it can be stored.");
          return;
        }
        ...
        try {
          ...
          setSubmitError(error instanceof Error ? error.message : "Unable to store this insight right now.");
        }
      }}
    />
  );
