...
Pairing is tunnel-first. The companion must decode Forge pairing payloads that include a recreated HTTPS tunnel transport, preserve that transport metadata across normalization and verified-session refreshes, and still keep known-host/manual HTTP setup available as an advanced route for LAN, local simulator, or Tailscale workflows.
