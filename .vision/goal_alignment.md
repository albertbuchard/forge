...
Forge should provide Courses as structured learning paths through first-class Concept entities. A concept owns its canonical definition, prerequisites, related concepts, cross-course appearances, mastery evidence, and spaced-review schedule. A course owns modules, daily lessons, activities, rubrics, and ordering without duplicating concept identity. Learners should move quickly between course paths and concept records, earn course points and grades from submitted work, and review due concepts across every enrolled course. Written mathematics, especially proofs, should be assessed through Forge's existing configured model connection against an explicit rubric and hidden instructor reference. If the model is unavailable, Forge must save the work and withhold the grade rather than fabricate feedback.
...
- a modular Course package contract, first-class Concept records, course and concept libraries, an immersive full-page lesson workspace, cross-course mastery and recall scheduling, course grades and points, and rubric-based written-response assessment through the existing Forge model provider
