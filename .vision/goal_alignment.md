...
Forge should provide **Agent Messages** as durable asynchronous mail between an authenticated owner and connected agents. A message may carry text, one original voice Artifact, or both; it preserves sender, initial and current recipient, timestamps, immutable provenance, audit history, and an idempotent lifecycle from delivery through claim, progress, acknowledgement, handling, failure, or forwarding. Atomic lease claims must prevent two polling agents from executing the same request, while exact retry receipts recover a committed operation after response loss. The only agent-readable Artifact bytes are the original voice for the specifically addressed, currently leased, nonterminal message. Web and native SwiftUI clients share default-agent routing and mailbox semantics. The iPhone outbox remains AES-GCM encrypted at rest and retries only during foreground or iOS-granted background-processing time, without promising a background deadline or silently transferring voice to a transcription provider.
