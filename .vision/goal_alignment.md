...
Forge should make secure pairing visible and low-friction without treating network reachability as identity. Every unlocked local-owner web session should surface pending browser, device, Companion, and agent requests as a notification that opens the exact review list in Settings → Agents. The owner reviews the request, enters its short code once, and approves once; the paired-client list must update immediately while activation finishes. `npx forge-memory pairing` provides the same local-owner workflow for terminal users. Remote browsers, phones, Codex, Hermes, OpenClaw, Claude Code, and other API clients then use scoped renewable credentials with silent renewal, revocation, and auditability. A remote browser must first try a surviving secure refresh cookie without depending on disposable browser storage, then try discoverable device-passkey restoration before asking for another pairing. After one normal browser pairing, Forge should immediately request user verification to register the device passkey by default, without adding another owner approval. Compatible browsers using the same credential provider and exact Forge HTTPS relying party may then restore the exact existing non-operator client authority—including installation, data root, client key, profile, scopes, owner epoch, and client epoch. Every restoration still requires user verification and must fail closed and revoke when that authority changes. Declining or lacking WebAuthn leaves the newly paired browser session and ordinary future pairing available. Tailscale remains a preferred private transport and optional additional filter, never an authorization bypass.
