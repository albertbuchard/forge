...
Forge should make secure pairing visible and low-friction without treating network reachability as identity. Every unlocked local-owner web session should surface pending browser, device, Companion, and agent requests as a notification that opens the exact review list in Settings → Agents. The owner reviews the request, enters its short code once, and approves once; the paired-client list must update immediately while activation finishes. `npx forge-memory pairing` provides the same local-owner workflow for terminal users. Remote browsers, phones, Codex, Hermes, OpenClaw, Claude Code, and other API clients then use scoped renewable credentials with silent renewal, revocation, and auditability. After a remote browser completes normal pairing, its user may explicitly register a discoverable WebAuthn credential for future trusted-device restoration. That credential must restore only the exact existing non-operator client authority—including installation, data root, client key, profile, scopes, owner epoch, and client epoch—must require user verification on every restoration, and must fail closed and revoke when that authority changes. The ordinary pairing path remains available when trusted-device registration is unsupported, declined, or revoked. Tailscale remains a preferred private transport and optional additional filter, never an authorization bypass.
