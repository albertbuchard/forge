...
The gamified progression layer is part of the same production stack. It uses the existing Fastify reward ledger and SQLite persistence, React 19 UI surfaces, Framer Motion celebrations, source-controlled TypeScript trophy/unlock catalog data, and project-owned raster sprites under `public/gamification/`.

The current progression direction is Forge Gamification: one canonical source-controlled catalog with 144 achievements and cosmetic unlocks, generated transparent atlas-backed trophy and cosmetic sprites, 30 Forge Smith mascot states, selectable gamification art themes, selected-user-first metric evaluation, and cosmetic equipment that never gates core Forge functionality. Forge does not maintain stale released-catalog forks; future catalog growth should happen through normal migrations and canonical ID changes when required.
...
- Gamification art is themeable. The released themes are `Fantasy` for dramatic high-pressure forge art, `Dramatic Smithie` for a warmer and funnier mascot-app style, and `Mind Locksmith` for a modern smart blacksmith-as-locksmith-of-the-mind metaphor grounded in Forge planning, memory, Psyche, health, and agent collaboration. Every theme must use transparent-ready cropped sprite outputs so the UI is not forced into square dark image cards.
  - `familyKey`
  - `familyLabel`
