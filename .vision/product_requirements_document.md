...
- Iroh-first mobile pairing: Fastify starts the repo-owned Rust `companion-iroh` host, creates a one-time payload with the desktop node id, pairing token, relay hint, and ALPN `forge-companion/1`, preserves the request API/UI URL as a direct HTTPS/Tailscale fallback for Iroh request timeouts, and keeps manual HTTP/TCP-only routing as an explicit advanced option
