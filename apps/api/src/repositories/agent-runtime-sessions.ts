...
import {
  ...
  type AgentRuntimeReconnectPlan,
  type AgentRuntimeSession,
  type AgentRuntimeSessionEvent,
  type AgentAction,
  type AgentRuntimeSessionStatus,
  type AgentRuntimeProvider,
  type CreateAgentRuntimeSessionEventInput,
  type CreateAgentRuntimeSessionInput,
  type DisconnectAgentRuntimeSessionInput,
  type HeartbeatAgentRuntimeSessionInput,
  type ReconnectAgentRuntimeSessionInput
} from "../types.js";
...
type AgentRuntimeSessionRow = {
  ...
  actor_label: string;
  connection_mode: AgentRuntimeSession["connectionMode"];
...
type AgentRuntimeSessionEventRow = {
  id: string;
  session_id: string;
  event_type: string;
  level: AgentRuntimeSessionEvent["level"];
  title: string;
  summary: string;
  metadata_json: string;
  created_at: string;
...
function toReconnectPlan(
  row: AgentRuntimeSessionRow
): AgentRuntimeReconnectPlan {
  ...
  if (row.provider === "openclaw") {
    return agentRuntimeReconnectPlanSchema.parse({
      summary:
        "Restart the OpenClaw gateway, then verify the Forge plugin can talk to the shared Forge runtime again.",
      commands: [
  ...
  if (row.provider === "hermes") {
    return agentRuntimeReconnectPlanSchema.parse({
      summary:
        "Ensure the Hermes Forge plugin is installed in Hermes' runtime environment, then bring Hermes back up so it can re-register its session.",
      commands: [
  ...
  if (row.provider === "claude") {
    return agentRuntimeReconnectPlanSchema.parse({
      summary:
        "Restart or resume Claude Code so the Forge MCP server starts again and reconnects to the shared Forge runtime.",
      commands: [
        "claude mcp list",
  ...
  return agentRuntimeReconnectPlanSchema.parse({
    summary:
      "Restart or resume the Codex session so the Forge MCP bridge launches again and re-registers with Forge.",
    commands: [
      "codex mcp list",
      "codex",
      `curl -s ${forgeBaseUrl}/api/v1/health`
...
function mapEvent(row: AgentRuntimeSessionEventRow): AgentRuntimeSessionEvent {
...
function countAgentActionsForSession(row: AgentRuntimeSessionRow) {
  if (!row.agent_id) {
    return 0;
  }
  ...
  const result = getDatabase()
    ...
    .get(row.agent_id, row.started_at, endAt) as { count: number };
  return result.count;
}

function listAgentActionsForSession(
  row: AgentRuntimeSessionRow,
  limit = 20
): AgentAction[] {
  if (!row.agent_id) {
...
function listRecentSessionEvents(sessionId: string, limit = 4) {
  const rows = getDatabase()
    ...
    .all(sessionId, limit) as AgentRuntimeSessionEventRow[];
  return rows.map(mapEvent);
}

function countSessionEvents(sessionId: string) {
  const result = getDatabase()
    .prepare(
      `SELECT COUNT(*) AS count
       FROM agent_runtime_session_events
...
function mapSession(row: AgentRuntimeSessionRow): AgentRuntimeSession {
...
function getSessionRowById(sessionId: string) {
  return getDatabase()
    .prepare(
      `SELECT
    ...
    .get(sessionId) as AgentRuntimeSessionRow | undefined;
}

function getSessionRowByCompositeKey(provider: string, sessionKey: string) {
  return getDatabase()
    .prepare(
      `SELECT
         id, agent_id, agent_label, agent_type, provider, session_key, session_label, actor_label,
    ...
    .get(provider, sessionKey) as AgentRuntimeSessionRow | undefined;
}
...
function resolveSessionRow(locator: {
  ...
  if (locator.provider?.trim() && locator.sessionKey?.trim()) {
    return getSessionRowByCompositeKey(
      locator.provider.trim(),
      locator.sessionKey.trim()
    );
  }
  return undefined;
}
...
function canonicalRuntimeAgentLabel(provider: AgentRuntimeProvider) {
...
function canonicalRuntimeDescription(provider: AgentRuntimeProvider) {
  return `${canonicalRuntimeAgentLabel(
    provider
  )} runtime agent with stable Forge identity and linked Kanban user.`;
}

function canonicalAgentUserSpec(provider: AgentRuntimeProvider) {
  if (provider === "openclaw") {
    return {
      ...
      description:
        "OpenClaw runtime actor linked to Forge agent identity and Kanban ownership.",
      accentColor: "#38bdf8"
    };
  }
  if (provider === "hermes") {
    return {
      ...
      displayName: "Hermes",
      description:
        "Hermes runtime actor linked to Forge agent identity and Kanban ownership.",
      accentColor: "#a78bfa"
    };
  }
  if (provider === "claude") {
  ...
  return {
    ...
    displayName: "Codex",
    description:
      "Codex runtime actor linked to Forge agent identity and Kanban ownership.",
    accentColor: "#22c55e"
  };
}

function deriveMachineKey(input: CreateAgentRuntimeSessionInput) {
...
function derivePersonaKey(input: CreateAgentRuntimeSessionInput) {
  return (
    normalizeIdentityPart(input.personaKey) ||
    normalizeIdentityPart(input.agentType) ||
    "default"
  );
}

function deriveAgentIdentityKey(input: CreateAgentRuntimeSessionInput) {
...
function linkAgentIdentityUsers(
  agentId: string,
  ...
  for (const userId of normalizedUserIds) {
    ...
    getDatabase()
      .prepare(
        `INSERT INTO agent_identity_users (agent_id, user_id, role, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?)
      ...
      .run(
        agentId,
        userId,
        userId === primaryUser.id ? "primary" : "linked",
        now,
        now
      );
  }
...
function upsertRuntimeAgentIdentity(input: CreateAgentRuntimeSessionInput) {
  ...
  if (existing) {
    getDatabase()
      ...
      .run(
        label,
        input.agentType || input.provider,
        identityKey,
        input.provider,
        machineKey,
        personaKey,
...
function insertSessionEvent(
  ...
  getDatabase()
    .prepare(
      `INSERT INTO agent_runtime_session_events (
    ...
    .run(
      ...
      sessionId,
      input.eventType,
      agentRuntimeEventLevelSchema.parse(input.level ?? "info"),
      input.title,
      input.summary?.trim() ?? "",
...
export function listAgentRuntimeSessions(): AgentRuntimeSession[] {
  const rows = getDatabase()
    .prepare(
      `SELECT
         id, agent_id, agent_label, agent_type, provider, session_key, session_label, actor_label,
...
export function getAgentRuntimeSessionHistory(sessionId: string) {
  ...
  const events = getDatabase()
    ...
    .all(sessionId) as AgentRuntimeSessionEventRow[];
  return {
    session,
    ...
    if (existing) {
      getDatabase()
        ...
        .run(
          agentId,
          agentLabel,
          parsed.agentType || parsed.provider,
          parsed.sessionLabel || parsed.sessionKey,
          parsed.actorLabel,
          parsed.connectionMode,
          parsed.status === "error" ? "error" : "connected",
          normalizeText(parsed.baseUrl),
          normalizeText(parsed.webUrl),
          normalizeText(parsed.dataRoot),
          normalizeText(parsed.externalSessionId),
          parsed.staleAfterSeconds,
          normalizeText(parsed.lastError),
          now,
          now,
          now,
...
export function heartbeatAgentRuntimeSession(
  input: HeartbeatAgentRuntimeSessionInput
) {
...
export function appendAgentRuntimeSessionEvent(
  input: CreateAgentRuntimeSessionEventInput
) {
...
export function reconnectAgentRuntimeSession(
  sessionId: string,
  input: ReconnectAgentRuntimeSessionInput
) {
  ...
  return runInTransaction(() => {
    ...
    recordActivityEvent({
      ...
      description:
        parsed.note || `${row.provider} session marked for reconnect.`,
      actor: null,
      source: "ui",
      metadata: {
        provider: row.provider,
...
export function disconnectAgentRuntimeSession(
  sessionId: string,
  input: DisconnectAgentRuntimeSessionInput
) {
