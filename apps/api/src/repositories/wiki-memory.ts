...
function isChunkableWikiIngestTextAsset(asset: WikiIngestAssetRow) {
...
function splitWikiIngestTextIntoChunks(sourceText: string, maxChars: number) {
...
async function splitLargeWikiIngestTextAsset(options: {
  ...
  const chunks = splitWikiIngestTextIntoChunks(
    sourceText,
    MAX_WIKI_INGEST_TEXT_CHUNK_CHARS
  );
  if (chunks.length <= 1) {
    return 0;
  }
  ...
  const baseName =
    path.basename(options.asset.file_name, extension) ||
    options.asset.file_name ||
    "source";
  const width = String(chunks.length).length;
  for (const [index, chunk] of chunks.entries()) {
    const chunkNumber = index + 1;
    const chunkFileName = `${baseName}-part-${String(chunkNumber).padStart(
      width,
      "0"
    )}-of-${String(chunks.length).padStart(width, "0")}${extension}`;
    const chunkHeader = [
...
export async function processWikiIngestJob(
  ...
  while (assetQueue().length > 0) {
    ...
    if (derivedChunkCount > 0) {
      totalFiles = Math.max(
        derivedChunkCount,
        totalFiles - 1 + derivedChunkCount
      );
      updateWikiIngestJob(jobId, {
        totalFiles,
      ...
      createWikiIngestLog(
        jobId,
        `Split ${nextAsset.file_name || "large text source"} into ${derivedChunkCount} chunks.`,
        "info",
        {
