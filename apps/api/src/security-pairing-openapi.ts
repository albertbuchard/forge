  ...
  return {
    ...
    "/api/v1/auth/master-password": {
      ...
      put: {
        ...
        description:
          "Direct-loopback local-owner operation. No password exists by default. Forge accepts any matching 15-to-128-character value; password-strength guidance is advisory and never adds composition or predictability rules. Forge stores only a peppered Argon2id verifier. Replacing an existing password requires the current password.",
        security: [{ operatorSession: [] }],
        requestBody: {
          required: true,
          content: jsonContent({
