...
import {
  ...
  nutritionBarcodeLookupSchema,
  nutritionBodyCheckinCreateSchema,
  nutritionExperimentCreateSchema,
  nutritionExperimentPatchSchema,
  nutritionFoodLogCreateSchema,
  nutritionFoodLogPatchSchema,
  nutritionFoodSearchSchema,
...
const AGENT_ONBOARDING_ENTITY_CATALOG_BASE = [
  ...
  },
  {
    ...
    fieldGuide: [
      ...
      },
      {
        ...
        required: false,
        description:
          "Board workflow lane. Keep this separate from lifecycle status.",
      ...
      },
      {
        ...
        required: false,
        description:
          "Human or bot user ids assigned to collaborate on the project.",
        defaultValue: []
      },
      {
        ...
        required: false,
        description:
          "Project PRD or brief: outcome, scope, constraints, and acceptance direction.",
        defaultValue: ""
      },
      {
        ...
        required: false,
        description:
          "Optional calendar/work-block rules that govern when this project may be scheduled."
      },
      {
        name: "targetPoints",
        type: "integer",
  ...
  },
  {
    entityType: "task",
    purpose:
      "A concrete actionable work item. The same stored family carries issue, task, and subtask levels; tasks are the one-session execution layer.",
    minimumCreateFields: ["title"],
    relationshipRules: [
    ...
    fieldGuide: [
      ...
      },
      {
        ...
        required: false,
        description:
          "Work-item level. Use issue for vertical slices, task for one focused execution session, and subtask for lightweight child steps.",
      ...
      },
      {
        ...
        required: false,
        description:
          "Human or bot user ids assigned to collaborate on this issue, task, or subtask.",
        defaultValue: []
      },
      {
        name: "goalId",
      ...
      },
      {
        ...
        required: false,
        description:
          "Parent issue id for level=task or parent task id for level=subtask. Issues should sit directly under projectId.",
        defaultValue: null,
        nullable: true
      },
      {
        ...
        required: false,
        description:
          "Execution instructions for the agent or human doing the one-session task.",
        defaultValue: ""
      },
      {
        name: "executionMode",
        ...
        required: false,
        description:
          "Whether the issue or task is intended for autonomous work or human-in-the-loop work.",
        enumValues: ["afk", "hitl"],
        defaultValue: null,
        nullable: true
      },
      {
        ...
        required: false,
        description:
          "Structured success criteria, usually Given/When/Then for issues or concrete done checks for tasks.",
        defaultValue: []
      },
      {
        name: "blockerLinks",
        ...
        required: false,
        description:
          "Optional blockers linked to Forge entities that make the work unable to proceed.",
        defaultValue: []
      },
      {
        ...
        required: false,
        description:
          "Closeout report for completed work items, especially tasks finished by agents.",
        defaultValue: null,
        nullable: true
      },
      {
...
function buildPreferredMutationPath(entityType: string) {
  ...
  switch (entityType) {
    ...
    case "entity_navigation":
      return "Use the dedicated Entity Navigation route family to list bounded pinned and recent records or touch one exact record the agent actually viewed. Pin and unpin remain human-operator-only actions in the Forge Action Bar.";
    case "movement":
...
function buildPreferredReadPath(entityType: string) {
  ...
  if (entityType in AGENT_ONBOARDING_READ_MODEL_ROUTES) {
    return AGENT_ONBOARDING_READ_MODEL_ROUTES[
      entityType as keyof typeof AGENT_ONBOARDING_READ_MODEL_ROUTES
    ];
  }
  switch (entityType) {
    case "wiki_page":
...
const AGENT_ONBOARDING_ENTITY_CATALOG = [
  ...
  enrichOnboardingEntityGuide({
    entityType: "entity_navigation",
    purpose:
      "The canonical cross-surface navigation surface for bounded pinned records and actor-scoped recently viewed records.",
    minimumCreateFields: [],
    relationshipRules: [
      "Entity Navigation is a bounded read-and-touch surface, not a stored batch CRUD entity.",
      "List pinned and recent records through the dedicated Entity Navigation route, then touch only the exact in-scope record the agent actually viewed.",
      "Pin and unpin are deliberate human-operator actions in the Forge Action Bar and are intentionally absent from agent route tools."
    ],
    searchHints: [
      "List the bounded navigation state before asking the user to reconstruct a title or entity type from memory.",
      "Ask which returned record they mean only when more than one plausible target exists.",
      "Keep unavailable pinned targets visible as unavailable; do not silently substitute a recent record.",
      "Touch only after the exact record has actually been opened or read."
    ],
    fieldGuide: []
  }),
  enrichOnboardingEntityGuide({
  ...
  enrichOnboardingEntityGuide({
    ...
    purpose:
      "The read-model nutrition and body-composition workspace for food ledger review, energy balance, sport-fuel context, body trend, appearance signals, gut comfort, cravings, subjective energy, hypotheses, and experiments.",
    minimumCreateFields: [],
    relationshipRules: [
      "Use this surface for review and interpretation.",
...
const AGENT_ONBOARDING_CONVERSATION_RULES = [
  ...
  "Use the Psyche hypothesis-versus-reflection gate before another broad question: reflect when no concrete cue, sequence, belief sentence, behavior, body signal, mode voice, payoff, cost, or consequence is visible; offer one discussable hypothesis when the cue, meaning, protection, payoff, or cost is visible and another broad question would make the user carry the interpretation alone. The hypothesis must change saveable wording, the primary Psyche container, links, flashcard/support action, or the next question; otherwise keep listening.",
...
const AGENT_ONBOARDING_ENTITY_CONVERSATION_PLAYBOOKS = [
  ...
  },
  {
    ...
    coachingGoal:
      "Review recent nights, regularity, score, stages, and recovery context before deciding whether one sleep_session needs enrichment.",
    askSequence: [
  ...
  },
  {
    ...
    coachingGoal:
      "Review workouts, training load, effort, activity types, and recovery context before deciding whether one workout_session needs enrichment.",
    askSequence: [
  ...
  },
  {
    ...
    openingQuestion:
      "Should I look across Forge for what most needs a next move, or is there something specific you already want to handle?",
    coachingGoal:
      "Read the bounded actor-scoped queue before making the user categorize it, explain why the leading item matters now, and manage only eligible items without hiding unresolved work.",
    askSequence: [
      "When the user asks what needs attention, read the active queue immediately instead of asking them to guess whether the answer is a decision, blocked task, review, or system repair.",
      "If the user names a specific item or action, refresh the current queue unless they already supplied a stable id from a current read, so the item and allowed actions are still truthful.",
      "Reflect why the most consequential item matters now using its severity, source, and current consequence; state uncertainty when the evidence does not establish urgency.",
      "Ask which item they want to handle only when more than one returned item plausibly fits their request.",
  ...
  {
    focus: "entity_navigation",
    openingQuestion:
      "Are you trying to reopen something pinned, something you viewed recently, or the record we just looked at?",
    coachingGoal:
      "Help the current actor recover one canonical record from bounded pins or recent history without asking them to reconstruct it from memory or implying that agents can pin records.",
    askSequence: [
      "List the bounded pinned and recent records before asking the user to reconstruct a title or entity type from memory.",
      "Ask which returned record they mean only when more than one plausible target matches their request.",
      "Open the exact canonical target and explain when a pinned target is unavailable instead of silently substituting another record.",
      "Touch a record only after the agent actually viewed that exact in-scope entity type and id.",
      "If the user wants to pin or unpin, open or point them to the Forge Action Bar so the deliberate choice remains human-controlled."
    ]
  },
  {
    focus: "life_event",
...
function buildPlaybookRouteInfo(focus: string) {
  ...
  const preferredReadPath =
    guide?.preferredReadPath ??
    (focus in AGENT_ONBOARDING_READ_MODEL_ROUTES
      ? AGENT_ONBOARDING_READ_MODEL_ROUTES[
          focus as keyof typeof AGENT_ONBOARDING_READ_MODEL_ROUTES
        ]
      : null);
  const apiAccessHint = [
    `Focus: ${focus}.`,
...
function buildQuestionFlowReadinessCheck(
  ...
  if (guide.entityType === "entity_navigation") {
    return "Ready to list through the published route key without guessing when the user wants to reopen something pinned or recently viewed. Ready to touch only after the agent actually viewed the exact in-scope entity type and id; pin and unpin remain human-operator-only Action Bar choices.";
  }
...
export const AGENT_ONBOARDING_TOOL_INPUT_CATALOG = [
  ...
  },
  {
    ...
    summary:
      "Read the nutrition and weight-loss surface with calorie ledger, protein/fiber targets, energy balance, body trend, subjective energy, gut comfort, aesthetic check-ins, hypotheses, experiments, and data-quality flags.",
    whenToUse:
      "Use when the operator wants food, body composition, sport-fueling, appearance, gut-health, or subjective-energy context before coaching or logging.",
    inputShape: "{ userIds?: string[] }",
    requiredFields: [],
    notes: [
  ...
  },
  {
    ...
    summary:
      "Parse natural-language food text or a photo description into a candidate Forge nutrition log through the openai-codex ChatGPT subscription connection.",
    whenToUse:
      "Use when the operator describes a meal imprecisely, wants AI-assisted food decomposition, or wants subjective food-effect notes captured alongside nutrition.",
    inputShape:
      "{ text?: string, imageDescription?: string, loggedAt?: string, mealLabel?: string, userIds?: string[] }",
    ...
    example:
      '{"text":"3 eggs, sourdough, Greek yogurt with honey, espresso; focused but bloated later","mealLabel":"breakfast"}'
  },
  {
...
function buildAgentOnboardingPayload(request: {
  ...
  return {
    ...
    tokenRecovery: {
      ...
      rotationSummary:
        "Forge reveals raw tokens once. If you lose one, rotate it or issue a new token through /api/v1/settings/tokens, then update the plugin config.",
      settingsSummary:
        "Token creation, rotation, and revocation all live on explicit settings routes so recovery stays operator-controlled without requiring a browser click."
    },
    requiredHeaders: {
  ...
  return {
    ...
    energyModel: {
      ...
      estimatedDailyEnergyBalanceKcal:
        weightLoss.energyModel.estimatedDailyEnergyBalanceKcal,
...
export async function buildServer(
  ...
  app.post("/api/v1/health/weight-loss/foods/barcode", async (request) => ({
    ...(await lookupNutritionBarcode(
      nutritionBarcodeLookupSchema.parse(request.body ?? {})
    ))
  }));
  ...
  app.post("/api/v1/mobile/pairing/heartbeat", async (request) =>
    heartbeatCompanionPairing(
      heartbeatCompanionPairingSchema.parse(request.body ?? {})
    )
  );
