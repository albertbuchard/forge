...
function createWatchObservationNote(
...
function projectPsycheEvent(
  pairing: PairingSessionLike,
  event: z.infer<typeof watchCaptureEventSchema>
): WatchProjectionResult | null {
  if (event.eventType === "trigger_capture") {
    ...
    const situation =
      stringPayloadValue(event.payload, "situation") ||
  ...
  if (
    event.eventType === "emotion_check_in" ||
...
function compactFallbackOptions(labels: string[], payloadKey: string) {
