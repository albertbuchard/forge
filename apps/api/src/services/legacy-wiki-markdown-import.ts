...
export type LegacyWikiMarkdownImportOptions = {
  dataRoot?: string;
  apply?: boolean;
  deleteFiles?: boolean;
...
function legacyBackupPath(dataRoot: string, backupLabel: string) {
...
export async function importLegacyWikiMarkdownToSqlite(
  input: LegacyWikiMarkdownImportOptions = {}
): Promise<LegacyWikiMarkdownImportResult> {
  const options = {
...
function hasStartupImportMarker() {
  const row = getDatabase()
    ...
    .get(startupImportMarkerId) as { id: string } | undefined;
  return Boolean(row);
}
