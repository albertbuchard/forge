...
function createFlowError(message: string) {
  ...
  while (index < script.length) {
    ...
    if (current === "'" || current === '"') {
      ...
      if (script[index] !== quote) {
        throw createFlowError(`Unterminated string in flow rule: ${script}`);
      }
      index += 1;
    ...
    throw createFlowError(`Unexpected token '${current}' in flow rule: ${script}`);
  }
...
function parseExpression(tokens: FlowToken[], script: string) {
  ...
  function expect<TType extends FlowToken["type"]>(type: TType) {
    ...
    if (!token) {
      throw createFlowError(`Expected '${type}' in flow rule: ${script}`);
    }
    return token;
  }
  ...
  function parsePrimary(): FlowExpression {
    const token = peek();
    if (!token) {
      throw createFlowError(`Unexpected end of flow rule: ${script}`);
    }
    if (consume("(")) {
    ...
    if (token.type === "identifier") {
      ...
      if (consume("(")) {
        ...
        if (
          ...
          throw createFlowError(`Unknown flow function '${identifier}' in rule: ${script}`);
        }
        return { kind: "call", name: identifier, args };
      }
      return { kind: "reference", itemId: identifier };
    }
    throw createFlowError(`Unexpected token '${token.type}' in flow rule: ${script}`);
  }
  ...
  if (index < tokens.length) {
    throw createFlowError(`Unexpected trailing tokens in flow rule: ${script}`);
  }
  return expression;
}
...
function resolveReference(expression: FlowExpression): string | null {
...
export function getQuestionnaireVisibilityState(
  ...
  const visibleItemIdsBySection = new Map<string, string[]>();

  for (const section of definition.sections) {
    const sectionVisible =
      !section.visibility ||
      evaluateRule(section.visibility.script, answers, visibleItemIds);
    if (!sectionVisible) {
      continue;
    }
    const sectionItemIds = section.itemIds.filter((itemId) => {
      ...
      const itemVisible =
        !item.visibility ||
        evaluateRule(item.visibility.script, answers, visibleItemIds);
      if (itemVisible) {
    ...
    if (sectionItemIds.length > 0) {
      ...
      visibleItemIdsBySection.set(section.id, sectionItemIds);
    }
  }
