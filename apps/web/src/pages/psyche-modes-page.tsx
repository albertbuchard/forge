...
export function PsycheModesPage() {
  ...
  const steps: Array<QuestionFlowStep<ModeProfileInput>> = [
        ...
        <>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {(
                ...
                <button
                  key={family}
                  type="button"
                  className={`rounded-[22px] border px-4 py-4 text-left transition ${value.family === family ? "border-[var(--ui-border-strong)] bg-[var(--ui-surface-2)] text-[var(--ui-ink-strong)]" : "border-[var(--ui-border-subtle)] bg-[var(--ui-surface-1)] text-[var(--ui-ink-soft)] hover:bg-[var(--ui-surface-hover)]"}`}
          ...
          >
            <Input
              ...
              placeholder="The Friday Vigil, The Scanner, The Good Son"
            />
          </FlowField>
          <FlowField
            label="Archetype"
          ...
          >
            <Input
              ...
              onChange={(event) => setValue({ archetype: event.target.value })}
              placeholder="Detached protector, vulnerable child, demanding critic"
            />
          </FlowField>
        </>
      )
    },
    {
      ...
      description:
        "Imagery, facial expression, and persona should make the mode instantly memorable.",
      render: (value, setValue) => (
        <>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            >
              <Textarea
                value={value.persona}
                onChange={(event) => setValue({ persona: event.target.value })}
                placeholder="Quiet, hyper-alert, always scanning for threat and trying to stay one step ahead."
              />
            </FlowField>
            <FlowField
              label="Imagery"
            ...
            >
              <Textarea
                value={value.imagery}
                onChange={(event) => setValue({ imagery: event.target.value })}
                placeholder="A night watchman under cold fluorescent light, pacing and checking every door twice."
              />
            </FlowField>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            >
              <Input
                value={value.symbolicForm}
                onChange={(event) =>
                  setValue({ symbolicForm: event.target.value })
                }
                placeholder="A locked visor, a fox, a steel shield"
              />
            </FlowField>
            <FlowField
            ...
            >
              <Input
                value={value.facialExpression}
                onChange={(event) =>
                  setValue({ facialExpression: event.target.value })
                }
                placeholder="Tight jaw, narrowed eyes, forced calm"
              />
            </FlowField>
          </div>
        </>
        ...
        <>
          <div className="grid gap-4 md:grid-cols-3">
            ...
            >
              <Textarea
                value={value.fear}
                onChange={(event) => setValue({ fear: event.target.value })}
                placeholder="If I relax, I will miss the threat and get hurt."
              />
            </FlowField>
            <FlowField
              label="Burden"
            ...
            >
              <Textarea
                value={value.burden}
                onChange={(event) => setValue({ burden: event.target.value })}
                placeholder="It believes it must never let the guard down, even when that means constant tension."
              />
            </FlowField>
            <FlowField
            ...
            >
              <Textarea
                value={value.protectiveJob}
                onChange={(event) =>
                  setValue({ protectiveJob: event.target.value })
                }
                placeholder="Keeps me prepared so I never get blindsided or look naive."
              />
            </FlowField>
          </div>
          <FlowField
          ...
          >
            <Textarea
              value={value.originContext}
              onChange={(event) =>
                setValue({ originContext: event.target.value })
              }
              placeholder="Started getting stronger during adolescence when mistakes were met with criticism and unpredictability."
            />
          </FlowField>
        </>
      )
      ...
      render: (value, setValue) => (
        <>
          ...
          <FlowField
            ...
            description="Choose the moves this mode tends to trigger, justify, or amplify."
          >
            <EntityLinkMultiSelect
  ...
  if (
    ...
    return (
      <LoadingState
        eyebrow="Modes"
        title="Loading modes"
        description="Getting modes, linked patterns, behaviors, and values ready."
      />
    );
  }
  ...
  return (
    <div className="grid gap-5">
      <PageHero
        entityKind="mode"
        title={
          <EntityName kind="mode" label="Modes" variant="heading" size="lg" />
        }
      ...
      >
        {modes.length === 0 ? (
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            {(
              Object.keys(groupedModes) as Array<keyof typeof groupedModes>
            ).map((family) => (
              <div
                key={family}
      ...
      <QuestionFlowDialog
        ...
        onSubmit={async () => {
          ...
          if (!parsed.success) {
            setSubmitError(
              "This mode still needs a family and a name before it can be saved."
            );
            return;
          }
          ...
          try {
            ...
            setSubmitError(
              error instanceof Error
                ? error.message
                : "Unable to save this mode right now."
            );
          }
        }}
      />
