...
type ComposerBodyProps = Omit<
  StartWorkComposerProps,
  "open" | "onOpenChange"
> & {
  onCancel: () => void;
};
...
function StartWorkComposerBody({
  ...
  const [timerMode, setTimerMode] = useState<"planned" | "unlimited">(
    "planned"
  );
      ...
      tasks
        ...
        .sort((left, right) => {
          if (left.time.hasCurrentRun !== right.time.hasCurrentRun) {
            return (
              Number(right.time.hasCurrentRun) - Number(left.time.hasCurrentRun)
            );
          }
  ...
  const plannedDurationSeconds =
    timerMode === "planned"
      ? Math.max(60, (Number.parseInt(plannedMinutes, 10) || 20) * 60)
      : null;
    ...
    <div className="grid gap-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h3 className="mt-2 font-display text-[clamp(1.2rem,1.8vw,1.65rem)] text-[var(--ui-ink-strong)]">
            Pick a task and start the timer
          </h3>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--ui-ink-soft)]">
            Use an existing task or create a new one quickly. Forge will move
            the task into in progress as soon as work starts.
          </p>
        </div>
          ...
          [
            {
              value: "existing",
              label: "Existing task",
              detail: "Search and start something that already exists."
            },
            {
              value: "new",
              label: "New quick task",
              detail:
                "Create a task with a title, short description, and project."
            }
          ] as const
        ).map((option) => {
            ...
            <button
              ...
              className={cn(
                ...
                selected
                  ...
                  : "border-[var(--ui-border-subtle)] bg-[var(--ui-surface-1)] text-[var(--ui-ink-soft)] hover:bg-[var(--ui-surface-hover)]"
              )}
          ...
          <label className="grid gap-2">
            ...
            <div className="relative">
              ...
              <Input
                ...
                placeholder="Search task, project, human, bot, or owner"
                className="pl-10"
              />
            </div>
          </label>

          <div className="grid max-h-[19rem] gap-2 overflow-y-auto rounded-[20px] bg-[var(--ui-surface-1)] p-2">
            {filteredTasks.length === 0 ? (
              <div className="rounded-[16px] border border-dashed border-[var(--ui-border-subtle)] px-4 py-8 text-center text-sm text-[var(--ui-ink-faint)]">
                ...
                return (
                  ...
                  >
                    <div className="flex min-w-0 items-start justify-between gap-3">
                      ...
                      <span className="text-[12px] text-[var(--ui-ink-faint)]">
                        {task.points} xp
                      </span>
                    </div>
                  </button>
        ...
        <div className="grid gap-3">
          ...
          {selectedProject ? (
            <div className="rounded-[16px] bg-[var(--ui-surface-1)] px-4 py-3 text-sm text-[var(--ui-ink-soft)]">
              This task will be created inside{" "}
      ...
      <div className="grid gap-2">
        <span className="type-label text-[var(--ui-ink-faint)]">Timer</span>
        <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_8rem]">
          {(
            [
              {
                value: "planned",
                label: "20 min planned",
                detail: "Pomodoro-style by default."
              },
              {
                value: "unlimited",
                label: "Unlimited",
                detail: "Keep tracking until you stop."
              }
            ] as const
          ).map((option) => {
            ...
            return (
              ...
              >
                <div className="font-medium">{option.label}</div>
                <div className="mt-1.5 text-sm leading-6 text-inherit/80">
                  {option.detail}
                </div>
              </button>
            );
      ...
      <div className="grid gap-3 rounded-[20px] border border-[var(--ui-border-subtle)] bg-[var(--ui-surface-1)] p-4">
        <div>
          ...
          <div className="mt-2 text-sm leading-6 text-[var(--ui-ink-soft)]">
            Optional. Record the repository and branch for this work session so
            the project board can show the live agent branch.
          </div>
        </div>
        <div className="grid gap-2 sm:grid-cols-2">
          <label className="grid gap-2">
      ...
      {errorMessage ? (
        <div className="rounded-[18px] bg-[var(--ui-danger-soft)] px-4 py-3 text-sm leading-6 text-[var(--danger)]">
          {errorMessage}
        </div>
      ) : null}

      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--ui-border-subtle)] pt-4">
        <div className="flex flex-wrap gap-2 text-[12px] text-[var(--ui-ink-faint)]">
          <div className="rounded-full bg-[var(--ui-surface-1)] px-3 py-2">
            Starts in progress
          </div>
          <div className="rounded-full bg-[var(--ui-surface-1)] px-3 py-2">
            {timerMode === "planned"
              ? `${toPlannedMinutes(plannedDurationSeconds)} min target`
              : "Unlimited timer"}
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
...
export function StartWorkComposer({
  open,
  onOpenChange,
  ...
  const composer = (
    <StartWorkComposerBody {...props} onCancel={() => onOpenChange(false)} />
  );
    ...
    <>
      {open && presentation !== "mobile_sheet" ? (
        <div className="hidden lg:block">
          <div className="rounded-[24px] border border-[var(--ui-border-subtle)] bg-[var(--ui-surface-modal)] p-4 shadow-[var(--ui-shadow-floating)]">
