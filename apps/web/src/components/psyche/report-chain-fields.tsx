...
function RemoveButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      type="button"
      className="rounded-full bg-[var(--ui-surface-2)] p-2 text-[var(--ui-ink-faint)] transition hover:bg-[var(--ui-surface-hover)] hover:text-[var(--ui-ink-strong)]"
      onClick={onClick}
    ...
    <div className="grid gap-3">
      ...
      <div className="grid gap-3">
        {items.map((item, index) => (
          <div
            key={`${title}-${index}`}
            className="flex items-start gap-3 rounded-[22px] border border-[var(--ui-border-subtle)] bg-[var(--ui-surface-1)] p-4"
          >
            <Input
              value={item}
              onChange={(event) =>
    ...
    <div className="grid gap-3">
      ...
      <div className="grid gap-3">
        {items.map((item) => (
          <div
            key={item.id}
            className="grid gap-4 rounded-[24px] border border-[var(--ui-border-subtle)] bg-[var(--ui-surface-1)] p-4"
          >
            <div className="flex items-start justify-between gap-3">
              <div className="grid flex-1 gap-4 md:grid-cols-[minmax(0,1fr)_9rem]">
                <label className="grid gap-2">
                  ...
                  <select
                    className="rounded-[18px] border border-[var(--ui-border-subtle)] bg-[var(--ui-surface-2)] px-4 py-3 text-sm text-[var(--ui-ink-strong)]"
            ...
            <label className="grid gap-2">
              <span className="text-sm text-[var(--ui-ink-soft)]">
                What was notable about this emotion?
              </span>
              <Textarea
                value={item.note}
                onChange={(event) =>
                ...
                <select
                  ...
                  onChange={(event) =>
                    onChange(
                      items.map((entry) =>
                        entry.id === item.id
                          ? { ...entry, parentMode: event.target.value }
                          : entry
                      )
                    )
                  }
                ...
                <select
                  ...
                  onChange={(event) =>
                    onChange(
                      items.map((entry) =>
                        entry.id === item.id
                          ? { ...entry, criticMode: event.target.value }
                          : entry
                      )
                    )
                  }
                ...
                <select
                  ...
                  onChange={(event) =>
                    onChange(
                      items.map((entry) =>
                        entry.id === item.id
                          ? { ...entry, beliefId: event.target.value || null }
                          : entry
                      )
                    )
                  }
        ...
        {items.map((item) => (
          ...
          >
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="grid gap-2">
                <span className="text-sm text-[var(--ui-ink-soft)]">
                  Linked existing behavior
                </span>
                <select
                  className="rounded-[18px] border border-[var(--ui-border-subtle)] bg-[var(--ui-surface-2)] px-4 py-3 text-sm text-[var(--ui-ink-strong)]"
                  value={item.behaviorId ?? ""}
                  onChange={(event) => {
                    const linked = behaviors.find(
                      (entry) => entry.id === event.target.value
                    );
                    onChange(
                      items.map((entry) =>
      ...
      <div className="grid gap-3">
        {items.map((item) => (
          ...
          >
            ...
            <label className="grid gap-2">
              <span className="text-sm text-[var(--ui-ink-soft)]">
                What was important about this shift?
              </span>
              <Textarea
                value={item.note}
                onChange={(event) =>
