        ...
        >
          {(section) => (
            <>
                ...
                <>
                  <FactsGrid
                    facts={[
                      ...
                      {
                        label: "Started",
                        value: formatDate(application.startedAt)
                      },
                      {
                        label: "Submitted",
                        value: formatDate(
                      ...
                      },
                      {
                        ...
                        value: formatDate(
                          application.expectedResponseAt,
                          "Not set"
                        )
                      },
                      {
                      ...
                      {
                        label: "Outcome",
                        value: application.outcome || "Open"
                      },
                      {
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-[var(--ui-ink-soft)]">
                            Reviewed{" "}
                            {formatDate(application.reapplicationReviewedAt)}
                          </p>
                        </div>
                        <Link
                  ...
                  <details className="rounded-[18px] border border-[var(--ui-border-subtle)] bg-[var(--ui-surface-1)] p-4">
                    <summary className="cursor-pointer text-sm font-semibold text-[var(--ui-ink-strong)]">
                      Submission proof and technical details
                    </summary>
                    <FactsGrid
                      facts={[
                        {
                        ...
                        },
                        {
                          ...
                          value:
                            application.confirmationReceipt || "Not recorded"
                        },
                        {
                          label: "Tracking reference",
                          value:
                            application.trackingIdentifier || "Not recorded"
                        }
                      ]}
                    />
                  </details>
              ...
              >
                {["materials", "process", "activity"].includes(section) ? (
                  <div className="grid gap-5">
                    {section === "activity" ? (
                      <>
                        <EventTimeline
                        ...
                        application.lessons ? (
                          <Card className="grid gap-5">
                            ...
                            <EvidenceList
                              title="Lessons"
                              items={
                                application.lessons ? [application.lessons] : []
                              }
                        ...
                        <Card>
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div>
                              ...
                              <p className="mt-1 text-xs text-[var(--ui-ink-soft)]">
                                Each material keeps the exact file version used;
                          ...
                          {application.artifactUses?.length ? (
                                ...
                                >
                                  <span className="min-w-0">
                                    ...
                                    <span className="block truncate text-xs text-[var(--ui-ink-soft)]">
                                      {readable(use.approvalState)}
                                    </span>
                                    <details className="mt-1 text-[10px] text-[var(--ui-ink-faint)]">
                                      <summary className="cursor-pointer">
                                        Technical details
                                      </summary>
                                      <span className="mt-1 block break-all font-mono">
                                        File ID {String(use.artifactId)} ·
                                        SHA-256{" "}
                                        {String(use.contentSha256 ?? "")}
                                      </span>
                                    </details>
                                  </span>
                            ...
                            <p className="mt-3 text-sm text-[var(--ui-ink-soft)]">
                              No curriculum vitae, cover letter, answer set,
                              portfolio, or receipt has been pinned yet.
                            </p>
                          )}
                        </Card>
                        <Card>
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div>
                              ...
                              <p className="mt-1 text-xs text-[var(--ui-ink-soft)]">
                                Exact employer wording with application-specific
                                reviewed answers.
                              </p>
                            </div>
                            <Button
                    ...
                    {section === "process" ? (
                      <>
                        ...
                        <Card>
                          ...
                          <p className="mt-1 text-xs text-[var(--ui-ink-soft)]">
                            Preview, approval, external action, and direct
                            evidence remain distinct.
                          </p>
                          <div className="mt-3 grid gap-2">
                            {application.transmissionPreviews?.map(
                              (preview) => (
                                <div
                                  key={preview.id}
                                  className="rounded-[16px] bg-[var(--ui-surface-2)] p-3"
                                >
                                  ...
                                  <details className="mt-2 text-[11px] text-[var(--ui-ink-soft)]">
                            ...
                            {!application.transmissionPreviews?.length ? (
                              <p className="text-sm text-[var(--ui-ink-soft)]">
                                No submission preview has been created.
                              </p>
                            ) : null}
                          </div>
                        </Card>
                ...
                {["summary", "process", "connections"].includes(section) ? (
                      ...
                      <Card>
                        ...
                        {application.status === "ready_to_submit" ? (
                          <div className="mt-3 rounded-[16px] border border-[color-mix(in_srgb,var(--warning)_30%,var(--ui-border-subtle))] bg-[color-mix(in_srgb,var(--warning)_7%,var(--ui-surface-1))] p-3 text-sm text-[var(--ui-ink-medium)]">
                            ...
                            Forge will not infer submission from a prepared
                            package. Build a preview, get final approval, send
                            it, and then record direct evidence.
                          </div>
                        ) : null}
                        {available.length ? (
