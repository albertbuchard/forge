...
export type NutritionTargetBuildInput = {
  sex: Sex;
  ageYears: number;
  currentWeightKg: number;
  calorieTarget: number;
...
function resolveMacroTargets({
  ...
  currentWeightKg: number;
  protein: number;
  savedFat: number | null;
}) {
  const maxFatForTarget = Math.max(0, (calorieTarget - protein * 4) / 9);
  const generatedFat = Math.max(
    0,
    Math.min(
      maxFatForTarget,
      (calorieTarget * 0.35) / 9,
      Math.max(currentWeightKg * 0.6, (calorieTarget * 0.25) / 9)
    )
  );
  const fat = savedFat ?? generatedFat;
...
function vitaminTargets(sex: Sex, ageYears: number) {
  ...
  return [
    row(
      ...
      sex === "male" ? 900 : 700,
      "ug RAE",
      "Retinol activity equivalent target.",
      "NASEM DRI"
    ),
    row(
      "vitamin_c",
    ...
    row(
      ...
      older ? 20 : 15,
      "ug",
      "Assumes limited sun exposure; older adults need more.",
      "NASEM DRI"
    ),
    row(
      "vitamin_e",
...
function mineralTargets(sex: Sex, ageYears: number) {
  const calcium =
    ageYears > 70 || (sex === "female" && ageYears >= 51) ? 1200 : 1000;
  const magnesium =
    sex === "male" ? (ageYears >= 31 ? 420 : 400) : ageYears >= 31 ? 320 : 310;
  const chromium =
    sex === "male" ? (ageYears >= 51 ? 30 : 35) : ageYears >= 51 ? 20 : 25;
  ...
  return [
    row(
      ...
      calcium,
      "mg",
      "Bone and muscle target; higher for older adults and women 51+.",
      "NASEM DRI"
    ),
    row(
      "iron",
      "Iron",
      sex === "female" && ageYears <= 50 ? 18 : 8,
      "mg",
      "Menstruating-age female target is higher.",
      "NASEM DRI"
    ),
    ...
    row(
      ...
      sex === "male" ? 3400 : 2600,
      "mg",
      "Current adult AI for non-pregnant adults.",
      "NASEM DRI"
    ),
    row(
      "sodium_limit",
      ...
      2300,
      "mg max",
      "Default daily ceiling unless medically individualized.",
      "U.S. Dietary Guidelines"
    )
  ];
}

export function buildNutritionTargetGroups(
  view: WeightLossViewData
): NutritionTargetGroups {
  ...
  const protein = asNumber(
    view.target.proteinGramsTarget,
    currentWeightKg * 1.8
  );
  const { carbs, fat } = resolveMacroTargets({
    calorieTarget,
    currentWeightKg,
    protein,
  ...
  return buildNutritionTargetGroupsFromValues({
    sex,
    ageYears,
    currentWeightKg,
    calorieTarget,
    proteinGramsTarget: protein,
    carbohydrateGramsTarget: carbs,
    fatGramsTarget: fat,
    fiberGramsTarget: fiber,
    activeBurnKcal,
...
export function buildNutritionTargetGroupsFromValues(
  input: NutritionTargetBuildInput
): NutritionTargetGroups {
  const {
    sex,
    ageYears,
  ...
  const trainingHours =
    activeBurnKcal != null
      ? Math.min(4, Math.max(0.1, activeBurnKcal / 500))
      : null;
  ...
  return {
    ...
    macros: [
      row(
        ...
        calorieTarget,
        "kcal",
        "Plan target from BMR, active burn, and chosen weekly rate.",
        "Forge plan"
      ),
      row(
        "protein",
        ...
        protein,
        "g",
        "Higher than the population RDA to preserve lean mass during weight change.",
        "ISSN + Forge plan"
      ),
      row(
        "carbohydrate",
        ...
        carbs,
        "g",
        "Remaining fuel after protein and fat. It may sit below the 130g/day adult DRI reference when needed to preserve the calorie target.",
        "NASEM DRI + Forge plan"
      ),
      row(
        "fat",
        "Total fat",
        fat,
        "g",
        "Uses the saved plan fat target when present; generated defaults aim for a practical floor around 0.6g/kg or 20-35% of calories when possible.",
        "NASEM AMDR + Forge plan"
      ),
      row(
        "fiber",
        "Fiber",
        fiber,
        "g",
        "Uses 14g per 1000 kcal as the plan target; the sex/age adult AI remains a reference or stretch value when calories are low.",
        "NASEM DRI"
      ),
      row(
        "saturated_fat",
      ...
      row(
        ...
        (calorieTarget * 0.1) / 4,
        "g max",
        "Keep below 10% of energy; lower is better when feasible.",
        "U.S. Dietary Guidelines"
      ),
      row(
        "linoleic_acid",
        "Linoleic acid",
        linoleicTarget(sex, ageYears),
        "g",
        "Essential n-6 fatty acid AI.",
        "NASEM DRI"
      ),
      row(
        "ala",
      ...
      row(
        ...
        sex === "male" ? 3.7 : 2.7,
        "L",
        "All beverages and food water before extra training replacement.",
        "NASEM DRI"
      )
    ],
    ...
    sportLosses: [
      row(
        ...
        trainingHours != null ? trainingHours : "n/a",
        "h/day",
        "Derived from active burn at roughly 500 kcal/h.",
        "Forge sport-loss model"
      ),
      row(
        "sweat_fluid",
        "Expected sweat fluid",
        fluidMin != null && fluidMax != null
          ? targetRange(fluidMin, fluidMax)
          : "n/a",
        "L/day",
        "Planning range uses 0.4-0.8 L/h; measure body mass change to calibrate.",
        "ACSM"
      ),
      row(
        "sweat_sodium",
        "Expected sweat sodium",
        sodiumMin != null && sodiumMax != null
          ? targetRange(sodiumMin, sodiumMax)
          : "n/a",
        "mg/day",
        "Sodium varies widely; this uses a conservative range around common sweat sodium values.",
        "ACSM + Sports Medicine review"
      ),
      row(
        "sweat_potassium",
        "Expected sweat potassium",
        potassiumMin != null && potassiumMax != null
          ? targetRange(potassiumMin, potassiumMax)
          : "n/a",
        "mg/day",
        "Potassium is usually much smaller than sodium but useful for long/hot sessions.",
        "Sports Medicine review"
      ),
      row(
        "gross_sport_equivalent",
        "Gross sport burn equivalent",
        grossSportWeightEquivalentKgPerWeek != null
