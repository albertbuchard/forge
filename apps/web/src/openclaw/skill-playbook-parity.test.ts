...
function extractOpenClawToolList(skill: string) {
  ...
  expect(
    start,
    "OpenClaw tool-list marker should exist"
  ).toBeGreaterThanOrEqual(0);
  const end = skill.indexOf(endMarker, start);
  expect(
    end,
    "OpenClaw tool-list end marker should exist"
  ).toBeGreaterThanOrEqual(0);
  return Array.from(
    skill.slice(start, end).matchAll(/`(forge_[a-z0-9_]+)`/g)
  ).map((match) => match[1]);
}
...
describe("forge skill playbook parity", () => {
  it("keeps the shared Psyche playbook aligned across agent surfaces", () => {
    ...
    expect(readRepoFile("plugins/hermes/psyche_entity_playbooks.md")).toBe(
      canonical
    );
    expect(
  ...
  it("keeps the agent-facing skills explicit about preferences, questionnaires, self-observation, and health surfaces", () => {
    ...
    expect(openclawSkill).toMatch(
      /Health side covers[\s\S]*training-load surface/i
    );
    ...
    expect(openclawSkill).toMatch(
      /caloriesKcal[\s\S]*proteinG[\s\S]*carbsG[\s\S]*fatG/
    );
    expect(openclawSkill).toMatch(/never save a name-only food/i);
    expect(openclawSkill).toMatch(
      /Batch CRUD is the default for simple entities|shared batch entity tools/i
    );
    expect(openclawSkill).toMatch(
    ...
    expect(openclawSkill).toMatch(
      /specialized domain surfaces are Movement, Life Events, Life Force, and Workbench/i
    );
    expect(openclawSkill).toMatch(
      /dedicated route families for domain actions/i
    );
    expect(openclawSkill).toMatch(
      /Movement is a specialized domain surface, not batch CRUD/i
    );
    expect(openclawSkill).toMatch(
      /Life Force is a specialized domain surface, not batch CRUD/i
    );
    expect(openclawSkill).toMatch(
      /Workbench is a specialized domain surface, not batch CRUD/i
    );
    for (const skill of [
      openclawSkill,
      hermesSkill,
      ...
      expect(skill).toMatch(
        /Keep the operation lane explicit across every entity family/i
      );
      expect(skill).toMatch(
      ...
      expect(skill).toMatch(
        /Batch CRUD is the default for normal stored entities/i
      );
      expect(skill).toMatch(/readModelOnlySurfaces/i);
      expect(skill).toMatch(
        /sleepOverview[\s\S]*sportsOverview[\s\S]*trainingLoad[\s\S]*sleep_overview[\s\S]*sports_overview[\s\S]*training_load/i
      );
      expect(skill).toMatch(
        /operatorOverview[\s\S]*operatorContext[\s\S]*calendarOverview/i
      );
      expect(skill).toMatch(
        /operator_overview[\s\S]*operator_context[\s\S]*calendar_overview/i
      );
      expect(skill).toMatch(
        /read-only(?: overview)? surfaces[\s\S]*not batch CRUD\s+entities/i
      );
      expect(skill).toMatch(
        /forge_get_operator_overview[\s\S]*forge_get_operator_context[\s\S]*forge_get_calendar_overview/i
      );
      expect(skill).toMatch(
        /wiki_page[\s\S]*calendar_connection[\s\S]*specialized CRUD surfaces/i
      );
      expect(skill).toMatch(
      ...
      expect(skill).toMatch(
        /Movement, Life Events, Life Force, and Workbench are specialized domain surfaces/i
      );
      ...
      expect(skill).toMatch(
        /live onboarding `routeKeys` list,[\s\S]*`methodRoutes` map[\s\S]*route-key tool schemas[\s\S]*exact route-key to method\/path/i
      );
      expect(skill).toMatch(
        /routeKeys[\s\S]*allowed names[\s\S]*methodRoutes[\s\S]*route-key-to-`METHOD \/api\/v1\/\.\.\.` source of truth/i
      );
      expect(skill).toMatch(
        /POST aggregate reads[\s\S]*Movement `selection`[\s\S]*DELETE\s+repair paths/i
      );
      expect(skill).toMatch(
      ...
      expect(skill).toMatch(
        /routeKey`, `query`, or `body`|routeKey, query, or body/i
      );
      expect(skill).toMatch(
        /schema and live\s+onboarding disagree[\s\S]*contract bug/i
      );
      expect(skill).toMatch(
      ...
      expect(skill).toMatch(
        /understand, decide, notice, remember, or\s+change later/i
      );
      expect(skill).toMatch(/minimum save-readiness\s+checkpoint/i);
      expect(skill).toMatch(
        /accepted wording[\s\S]*meaningful body[\s\S]*route lane/i
      );
      expect(skill).toMatch(
        /write, read, run, or update[\s\S]*instead of collecting optional\s+fields/i
      );
      expect(skill).toMatch(
      ...
      expect(skill).toMatch(
        /belief sentence[\s\S]*functional loop[\s\S]*flashcard cue\/message/i
      );
      ...
      expect(skill).toMatch(
        /Search before creating duplicates|check for duplicates/i
      );
      ...
      expect(skill).toMatch(
        /Movement all-time read[\s\S]*"routeKey":"allTime"/i
      );
      expect(skill).toMatch(
        /Movement timeline read[\s\S]*"routeKey":"timeline"/i
      );
      expect(skill).toMatch(
        /Movement selection aggregate[\s\S]*"routeKey":"selection"[\s\S]*"body"[\s\S]*"placeIds"/i
      );
      expect(skill).not.toMatch(
        /Movement selection aggregate:\s*\n\s*`\{"routeKey":"selection","query"/i
      );
      expect(skill).toMatch(
        /Movement trip detail[\s\S]*"routeKey":"tripDetail"/i
      );
      expect(skill).toMatch(
        /Movement settings read[\s\S]*"routeKey":"settings"/i
      );
      expect(skill).toMatch(
        /Movement settings update[\s\S]*"routeKey":"settingsUpdate"/i
      );
      expect(skill).toMatch(
        /Movement known-place creation[\s\S]*"routeKey":"placeCreate"/i
      );
      expect(skill).toMatch(
        /Movement known-place update[\s\S]*"routeKey":"placeUpdate"/i
      );
      expect(skill).toMatch(
        /GET \/api\/v1\/movement\/settings[\s\S]*PATCH \/api\/v1\/movement\/settings/i
      );
      expect(skill).toMatch(
        /passive capture[\s\S]*publish mode[\s\S]*retention mode/i
      );
      expect(skill).toMatch(
        /Movement missing-stay correction[\s\S]*"routeKey":"userBoxPreflight"[\s\S]*"routeKey":"userBoxCreate"/i
      );
      expect(skill).toMatch(
        /Movement saved-overlay update[\s\S]*"routeKey":"userBoxUpdate"/i
      );
      expect(skill).toMatch(
        /Movement saved-overlay delete[\s\S]*"routeKey":"userBoxDelete"/i
      );
      expect(skill).toMatch(/Life Force overview[\s\S]*"routeKey":"overview"/i);
      expect(skill).toMatch(
        /Life Force profile edit[\s\S]*"routeKey":"profile"/i
      );
      expect(skill).toMatch(
        /Life Force weekday template edit[\s\S]*"routeKey":"weekdayTemplate"/i
      );
      expect(skill).toMatch(
        /Life Force fatigue signal[\s\S]*"routeKey":"fatigueSignal"/i
      );
      expect(skill).toMatch(
        /Workbench flow catalog[\s\S]*"routeKey":"listFlows"/i
      );
      expect(skill).toMatch(
        /Workbench flow detail[\s\S]*"routeKey":"flowDetail"/i
      );
      expect(skill).toMatch(
        /Workbench box catalog[\s\S]*"routeKey":"boxCatalog"/i
      );
      expect(skill).toMatch(
        /Workbench flow creation[\s\S]*"routeKey":"createFlow"/i
      );
      expect(skill).toMatch(
        /Workbench flow edit[\s\S]*"routeKey":"updateFlow"/i
      );
      expect(skill).toMatch(
        /Workbench flow deletion[\s\S]*"routeKey":"deleteFlow"/i
      );
      expect(skill).toMatch(
        /Workbench run history[\s\S]*"routeKey":"runHistory"/i
      );
      expect(skill).toMatch(
        /Workbench run detail[\s\S]*"routeKey":"runDetail"/i
      );
      expect(skill).toMatch(/Workbench run nodes[\s\S]*"routeKey":"runNodes"/i);
      expect(skill).toMatch(
        /Workbench node result[\s\S]*"routeKey":"nodeResult"/i
      );
      expect(skill).toMatch(
        /Workbench published output[\s\S]*"routeKey":"publishedOutput"/i
      );
      expect(skill).toMatch(
        /Workbench latest node output[\s\S]*"routeKey":"latestNodeOutput"/i
      );
      expect(skill).toMatch(
        /Workbench run execution[\s\S]*"routeKey":"runFlow"/i
      );
      expect(skill).toMatch(
        /Workbench one-off input execution[\s\S]*"routeKey":"runByPayload"/i
      );
      expect(skill).toMatch(
        /Workbench flow chat follow-up[\s\S]*"routeKey":"chatFlow"/i
      );
      expect(skill).toMatch(
        /stable input contract[\s\S]*intended\s+published output[\s\S]*smallest structural change/i
      );
      expect(skill).toMatch(
        /deletion[\s\S]*published outputs or run\s+history need preservation/i
      );
      expect(skill).toMatch(/POST \/api\/v1\/workbench\/flows\/:id\/chat/i);
      expect(skill).toMatch(/new run, note, or generic entity update/i);
    }
    expect(openclawSkill).toMatch(
      /conversation is clearly about a Forge entity or domain surface/i
    );
    ...
    expect(openclawSkill).toMatch(
      /Minimum-field checkpoint, not a question script/i
    );
    ...
    expect(openclawSkill).not.toMatch(
      /Use this intake map when the user agrees/i
    );
    expect(openclawSkill).not.toMatch(/\n\s*Ask:\n/);

    expect(hermesSkill).toMatch(
      /high-level batch routes for basic Preferences CRUD/i
    );
    expect(hermesSkill).toMatch(
      /high-level batch routes for basic questionnaire CRUD/i
    );
    expect(hermesSkill).toMatch(
      /Health side covers[\s\S]*training-load surface/i
    );
    expect(hermesSkill).toMatch(
      /Read-model-only surfaces:[^\n]*training load/i
    );
    expect(hermesSkill).toMatch(
      /add, update, review, compare, navigate, link, or run/i
    );
    expect(hermesSkill).toMatch(/exact correction in usable language/i);
    expect(hermesSkill).toMatch(/stop asking and write/i);
    expect(hermesSkill).toMatch(
      /review, compare, inspect, or navigate an existing Forge/i
    );
    ...
    expect(codexSkill).toMatch(
      /hundreds of individual CRUD routes|route jungle/i
    );
    ...
    expect(codexSkill).toMatch(
      /\/api\/v1\/movement\/automatic-boxes\/:id\/invalidate/i
    );
    ...
    expect(codexSkill).toMatch(
      /timeline,[\s\S]*overlay,[\s\S]*weekday\s+template,[\s\S]*published output,[\s\S]*run detail,[\s\S]*node result/i
    );
    expect(hermesSkill).toMatch(
      /timeline,[\s\S]*overlay,[\s\S]*weekday\s+template,[\s\S]*published output,[\s\S]*run detail,[\s\S]*node result/i
    );
    ...
    expect(hermesSkill).toMatch(
      /specialized domain surfaces are Movement,[\s\S]*Life Force,[\s\S]*Workbench/i
    );
    expect(hermesSkill).toMatch(
      /dedicated route families instead of[\s\S]*batch CRUD/i
    );
    ...
    expect(codexSkill).toMatch(
      /specialized Movement, Life Events, Life Force, and Workbench domain surfaces/i
    );
    expect(codexSkill).toMatch(
      /Movement, Life Events, Life Force, and Workbench use dedicated route[\s\S]*batch CRUD/i
    );
    ...
    for (const skill of [openclawSkill, hermesSkill, codexSkill]) {
      ...
      expect(skill).toMatch(
        /flashcard \{ message \}|`flashcard`[\s\S]*message/i
      );
    ...
    expect(
      ...
      /custom food[\s\S]*calories plus protein, carbohydrate, and fat/i
    );
  });
  ...
  it("keeps OpenClaw data-root guidance focused on effective runtime config", () => {
    ...
    expect(dataLocationSlice).toMatch(
      /Do not merge side databases unless an ID\/content-level audit proves/i
    );
    expect(dataLocationSlice).not.toMatch(
      /~\/\.openclaw\/extensions\/forge-openclaw-plugin\/forge\.sqlite/
    );
    expect(dataLocationSlice).not.toMatch(
      /<repo>\/plugins\/openclaw\/forge\.sqlite/
    );
  });
  ...
  it("keeps OpenClaw and Hermes explicit about habit semantics and the shared check-in path", () => {
    ...
    for (const skill of [openclawSkill, hermesSkill]) {
      ...
      expect(skill).toMatch(
        /bad habit was[\s\S]*resisted|habit was[\s\S]*resisted/i
      );
      ...
      expect(skill).not.toMatch(
        /direct raw calls? to \/api\/v1\/habits\/:id\/check-ins[\s\S]*preferred/i
      );
    }
  });
  ...
  it("keeps the canonical playbooks focused on guided, one-lane questioning", () => {
    ...
    expect(entityPlaybook).toMatch(
      /Do not bounce to the Forge UI, a browser session, or a generic web route/i
    );
    expect(entityPlaybook).toMatch(/## Tag/);
    expect(entityPlaybook).toMatch(/offer a tentative title or summary/i);
    expect(entityPlaybook).toMatch(
      /reflect what the user is trying to[\s\S]*preserve, change, or make true/i
    );
    expect(entityPlaybook).toMatch(
      /short reflection -> one orienting question/i
    );
    expect(entityPlaybook).toMatch(/what would you be trying to make true/i);
    expect(entityPlaybook).toMatch(
      /already answered the usual opening question/i
    );
    expect(entityPlaybook).toMatch(/stop exploring broadly/i);
    expect(entityPlaybook).toMatch(
      /Do not over-warm or over-therapize logistical records/i
    );
    expect(entityPlaybook).toMatch(
      /Lead with what the user is trying to preserve, change, resolve, or make true/i
    );
    expect(entityPlaybook).toMatch(
      /When the user is vague, ask for the smallest real example, desired outcome, or stake/i
    );
    expect(entityPlaybook).toMatch(
      /When the user is clear, say what the record seems to be becoming/i
    );
    ...
    expect(entityPlaybook).toMatch(
      /Prefer "what", "when", and "how" before "why"/i
    );
    expect(entityPlaybook).toMatch(
    ...
    expect(entityPlaybook).toMatch(
      /emotionally meaningful vocabulary records/i
    );
    ...
    expect(entityPlaybook).toMatch(
      /confirm only the missing route-selecting detail[\s\S]*then act/i
    );
    expect(entityPlaybook).toMatch(
    ...
    expect(entityPlaybook).toMatch(
      /`movement`[\s\S]*specialized domain surface/i
    );
    expect(entityPlaybook).toMatch(
      /`life_force`[\s\S]*specialized domain surface/i
    );
    expect(entityPlaybook).toMatch(
      /`workbench`[\s\S]*specialized domain surface/i
    );
    expect(entityPlaybook).toMatch(
      /`psyche_value`[\s\S]*`emotion_definition`[\s\S]*normal stored entities for API purposes/i
    );
    expect(entityPlaybook).toMatch(
    ...
    expect(entityPlaybook).toMatch(
      /behavior_pattern` for recurring loops|Use `behavior_pattern` for a recurring loop/i
    );
    expect(entityPlaybook).toMatch(
      /wiki_page` when the user wants durable memory|Use `wiki_page` when the user wants durable memory/i
    );
    expect(entityPlaybook).toMatch(
      /candidate label[\s\S]*what kinds of moments belong inside it/i
    );
    expect(entityPlaybook).toMatch(
      /keep it provisional[\s\S]*future use are clear/i
    );
    expect(entityPlaybook).toMatch(
      /When the record is already clear enough to save, save it/i
    );
    expect(entityPlaybook).toMatch(
      /if the user confirms it, stop asking and save/i
    );
    ...
    expect(psychePlaybook).toMatch(
      /Before the next question, reflect back what you just heard/i
    );
    expect(psychePlaybook).toMatch(/formulation work/i);
    expect(psychePlaybook).toMatch(
      /what the experience is[\s\S]*trying to[\s\S]*protect, prevent, or hold onto/i
    );
    expect(psychePlaybook).toMatch(
      /choose the one that most[\s\S]*improves understanding/i
    );
    ...
    expect(psychePlaybook).toMatch(
      /`behavior_pattern`[\s\S]*cue[\s\S]*short-term payoff[\s\S]*long-term cost/i
    );
    expect(psychePlaybook).toMatch(
      /`belief_entry`[\s\S]*rule, prediction, or self\/other\/world sentence/i
    );
    expect(psychePlaybook).toMatch(
      /`mode_profile`[\s\S]*protective job[\s\S]*feared danger[\s\S]*burden/i
    );
    expect(psychePlaybook).toMatch(
      /`flashcard`[\s\S]*cue[\s\S]*urge sentence[\s\S]*brief message/i
    );
    expect(psychePlaybook).toMatch(
      /`emotion_definition`[\s\S]*body signature[\s\S]*urge[\s\S]*warning/i
    );
    expect(psychePlaybook).toMatch(
    ...
    expect(psychePlaybook).toMatch(
      /whether it feels true, too sharp, or still misses something important/i
    );
    expect(psychePlaybook).toMatch(
      /one brief reflection[\s\S]*one missing-detail question/i
    );
    expect(psychePlaybook).toMatch(
      /what does it seem to prove in that moment/i
    );
    expect(psychePlaybook).toMatch(/Psyche update loop/i);
    expect(psychePlaybook).toMatch(
      /what the old wording was trying to[\s\S]*hold/i
    );
    expect(psychePlaybook).toMatch(
      /revise the whole formulation, or only the part that now feels inaccurate/i
    );
    expect(psychePlaybook).toMatch(
      /newly true, newly visible, or newly inaccurate/i
    );
    expect(psychePlaybook).toMatch(
      /charged episode[\s\S]*before you rename the durable|recent charged episode[\s\S]*before you re-check the durable/i
    );
    ...
    expect(psychePlaybook).toMatch(
      /choose the clearest primary[\s\S]*container first/i
    );
    expect(psychePlaybook).toMatch(/stop deepening and[\s\S]*name it cleanly/i);
    expect(psychePlaybook).toMatch(
      /What happened the last time this pattern showed up/i
    );
    expect(psychePlaybook).toMatch(
      /What did you find yourself doing the last time this move showed up/i
    );
    expect(psychePlaybook).toMatch(
      /When that reaction hits, what does it start telling you about you, them, or what happens next/i
    );
    expect(psychePlaybook).toMatch(
      /What exact urge sentence or situation should make this card appear/i
    );
    expect(psychePlaybook).toMatch(
      /What kind of moment keeps happening that you want future reports to name the same way each time/i
    );
    expect(psychePlaybook).toMatch(
      /When this feeling is present, what tells you it is this feeling and not a nearby one/i
    );
  ...
  it("covers representative user requests for preferences, questionnaires, self-observation, calendar, and health work", () => {
    ...
    const fakeRequests = [
      ...
      },
      {
        ...
        questioning: [
          /For straightforward logistical entities such as tasks, calendar events/i
        ]
      },
      {
        request:
  ...
  it("keeps onboarding source and OpenAPI schema aligned for specialized surfaces and action-heavy flows", () => {
    ...
    expect(appSource).toMatch(
      /routeKeys:\s*\[\s*"overview",\s*"profile",\s*"weekdayTemplate",\s*"fatigueSignal"\s*\]/
    );
    expect(appSource).toMatch(/methodRoutes:/);
    expect(appSource).toMatch(
      /selection:\s*"POST \/api\/v1\/movement\/selection"/
    );
    expect(appSource).toMatch(/overview:\s*"GET \/api\/v1\/life-force"/);
    expect(appSource).toMatch(
      /runFlow:\s*"POST \/api\/v1\/workbench\/flows\/:id\/run"/
    );
    expect(appSource).toMatch(
      /flowDetail:\s*"GET \/api\/v1\/workbench\/flows\/:id"/
    );
    expect(appSource).toMatch(
      /runHistory:\s*"GET \/api\/v1\/workbench\/flows\/:id\/runs"/
    );
    ...
    expect(appSource).toMatch(
      /another agent could follow the same path without guessing/i
    );
    expect(appSource).toMatch(
