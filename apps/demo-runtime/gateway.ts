...
const demoRoot = path.resolve(
  process.env.FORGE_DEMO_ROOT?.trim() ||
    path.join(os.tmpdir(), "forge-public-demo")
);
...
function secureRequest(request: IncomingMessage) {
  return Boolean(
    (request.socket as IncomingMessage["socket"] & { encrypted?: boolean })
      .encrypted || request.headers["x-forwarded-proto"] === "https"
  );
}
  ...
  return await new Promise<number>((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      if (!address || typeof address === "string")
        return reject(new Error("Could not allocate a demo port."));
...
async function createSession() {
  if (sessions.size >= maxSessions) {
    throw Object.assign(
      new Error(
        "The public demo is at its 20-session capacity. Try again after an existing session expires."
      ),
      { statusCode: 503 }
    );
  }
  ...
  const child = spawn(
    ...
    [
      "--import",
      "tsx",
      "apps/demo-runtime/session-server.ts",
      "--root",
      root,
      "--port",
      String(childPort)
    ],
    {
      ...
      env: {
        ...process.env,
        FORGE_DATA_ROOT: root,
        HOST: "127.0.0.1",
        PORT: String(childPort),
        FORGE_DISCOVERY_DISABLED: "1"
      },
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(
        () => reject(new Error("Demo session startup timed out.")),
        20_000
      );
      let output = "";
      ...
      child.once("exit", (code) => {
        clearTimeout(timeout);
        reject(
          new Error(`Demo session exited during startup (${code ?? "signal"}).`)
        );
      });
    });
  } catch (error) {
    ...
    throw Object.assign(
      new Error(
        "The isolated demo session could not start. Try again shortly."
      ),
      { statusCode: 503 }
    );
  }
...
async function resolveSession(
  ...
  if (
    !session ||
    decoded!.createdAt !== session.createdAt ||
    Date.now() - session.createdAt > sessionTtlMs
  ) {
  ...
  if (session.requests.length >= 120)
    throw Object.assign(
      new Error("Demo request limit reached. Wait one minute."),
      { statusCode: 429 }
    );
...
async function readBody(request: IncomingMessage) {
  ...
  for await (const chunk of request) {
    ...
    if (bytes > maxBodyBytes)
      throw Object.assign(new Error("Demo requests are limited to 1 MiB."), {
        statusCode: 413
      });
    chunks.push(buffer);
  }
...
async function serveStatic(
  ...
  const relative = url.pathname.startsWith("/forge/")
    ? url.pathname.slice("/forge/".length)
  ...
  response.writeHead(200, {
    ...
    "Content-Security-Policy":
      "default-src 'self'; img-src 'self' data: blob:; style-src 'self' 'unsafe-inline'; font-src 'self' data:; worker-src 'self' blob:; connect-src 'self'"
  });
  ...
  try {
        ...
        JSON.stringify({
          demo: {
            ...
            limits: {
              ...
              requestBytes: maxBodyBytes
            },
            allowedMutation: "sample_task_status_only"
          }
        })
      );
      return;
    ...
    if (!demoRouteAllowed(request.method ?? "GET", url, body)) {
      ...
      response.end(
        JSON.stringify({
          error:
            "This public demonstration exposes sample-data reads and sample task status changes only.",
          code: "demo_route_denied"
        })
      );
      return;
    }
    ...
    const proxied = proxyRequest(
      ...
      (upstream) => {
        const responseHeaders = markDemoProxyResponse(upstream.headers);
        ...
        if (combinedCookies.length > 0)
          responseHeaders["set-cookie"] = combinedCookies;
    ...
    proxied.on("error", () => {
      ...
      response.end(
        JSON.stringify({ error: "The isolated demo session is restarting." })
      );
    });
    proxied.end(body);
  } catch (error) {
...
const cleanup = setInterval(() => {
  ...
  for (const session of sessions.values())
    if (
      now - session.lastSeenAt > idleTtlMs ||
      now - session.createdAt > sessionTtlMs
    )
...
server.listen(port, host, () =>
  console.log(`Forge public demo listening on http://${host}:${port}/forge/`)
);
