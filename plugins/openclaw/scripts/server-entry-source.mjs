...
const builtRuntimeEntry = path.join(
  packageRoot,
  "dist",
  "server",
  "apps",
...
const useDevRuntime =
  devModeFlag === "1" || devModeFlag === "true" || devModeFlag === "yes";
...
if (!useDevRuntime) {
  ...
  if (
    !existsSync(devRuntimeEntry) ||
    !existsSync(devDataRootWrapper) ||
    !existsSync(tsxCliEntry)
  ) {
    throw new Error(
  ...
  console.log(
    "[forge-openclaw-plugin] starting source-backed dev runtime on port",
  ...
  await new Promise((resolve, reject) => {
    ...
    child.once("exit", (code, signal) => {
      ...
      reject(
        new Error(
          `Forge OpenClaw dev runtime exited with code ${code ?? "unknown"}.`
        )
      );
    });
  });
