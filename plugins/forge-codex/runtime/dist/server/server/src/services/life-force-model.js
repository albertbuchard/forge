...
export function resolveTaskExpectedDurationSeconds(plannedDurationSeconds) {
    return Math.max(60, plannedDurationSeconds ?? DEFAULT_TASK_EXPECTED_DURATION_SECONDS);
}
export function resolveActionCostBand(totalCostAp) {
    if (totalCostAp <= 10) {
        return "tiny";
    }
    if (totalCostAp <= 35) {
...
export function resolveBandTotalCostAp(costBand) {
    switch (costBand) {
        case "tiny":
            return 12;
        case "light":
...
export function buildDefaultTaskActionProfile(input) {
    const expectedDurationSeconds = resolveTaskExpectedDurationSeconds(input.expectedDurationSeconds);
    const totalCostAp = Math.max(1, input.totalCostAp ??
        resolveBandTotalCostAp(input.costBand) ??
        DEFAULT_TASK_TOTAL_AP);
    const sustainRateApPerHour = (totalCostAp / expectedDurationSeconds) * 3600;
...
export function buildTaskActionPointSummary(input) {
    const expectedDurationSeconds = resolveTaskExpectedDurationSeconds(input.plannedDurationSeconds);
...
export function buildTaskSplitSuggestion(input) {
    ...
    const projectedTotalSeconds = Math.max(totalTrackedSeconds, input.projectedTotalSeconds ?? totalTrackedSeconds);
...
export function normalizeCurveToBudget(points, budgetAp) {
...
export function computeCurveHandleMaxRate(points, index, budgetAp, minPointGapMinutes = MIN_LIFE_FORCE_POINT_GAP_MINUTES) {
    ...
    const leftHours = Math.max(minPointGapMinutes / 60, (current.minuteOfDay - previous.minuteOfDay) / 60);
    const rightHours = Math.max(minPointGapMinutes / 60, (next.minuteOfDay - current.minuteOfDay) / 60);
    return Math.max(0, (2 * (budgetAp - fixedArea) -
        previous.rateApPerHour * leftHours -
        next.rateApPerHour * rightHours) /
        (leftHours + rightHours));
}
export function buildDefaultSplitTitles(title) {
...
export function interpolateCurveRate(points, minuteOfDay) {
    if (points.length === 0) {
        return 0;
    }
