...
export const updateWorkoutSessionSchema = createWorkoutSessionSchema
    .omit({ userId: true })
    .partial();
let legacyAppleSleepRepairDatabase = null;
function nowIso() {
...
function unionDurationSeconds(intervals) {
    ...
    return merged.reduce((total, interval) => total + durationSecondsBetween(interval.startedAt, interval.endedAt), 0);
}
...
function localDateKeyForTimezone(value, timeZone) {
...
function mergeSleepAnnotationPayloads(annotations, links) {
    const qualitySummary = Array.from(new Set(annotations
        .map((entry) => typeof entry?.qualitySummary === "string"
        ? entry.qualitySummary.trim()
        : "")
        .filter(Boolean))).join("\n\n");
    const notes = Array.from(new Set(annotations
        .map((entry) => typeof entry?.notes === "string" ? entry.notes.trim() : "")
        .filter(Boolean))).join("\n\n");
    const tags = mergeStringLists(...annotations.map((entry) => Array.isArray(entry?.tags) ? entry?.tags : []));
    const mergedLinks = mergeHealthLinks(...links, ...annotations.map((entry) => Array.isArray(entry?.links)
        ? entry?.links
        : []));
    return {
        qualitySummary,
...
function sleepMinutesOfDay(value, mode, timeZone = "UTC") {
...
function computeSleepTimingMetrics(input) {
    ...
    const bedtimeReference = average(rows.map((row) => sleepMinutesOfDay(row.started_at, "bedtime", sourceTimezone)));
    const wakeReference = average(rows.map((row) => sleepMinutesOfDay(row.ended_at, "wake", sourceTimezone)));
    const bedtimeConsistencyMinutes = Math.round(Math.abs(sleepMinutesOfDay(input.startedAt, "bedtime", sourceTimezone) -
        bedtimeReference));
    const wakeConsistencyMinutes = Math.round(Math.abs(sleepMinutesOfDay(input.endedAt, "wake", sourceTimezone) - wakeReference));
...
function computeRegularityScore(startedAt, timeZone = "UTC") {
...
function mapSleepSession(row) {
    return {
        ...
        sourceMetrics: safeJsonParse(row.source_metrics_json, {}),
...
function mapSleepSegment(row) {
    return {
        ...
        qualityKind: row.quality_kind,
        sourceRecordIds: safeJsonParse(row.source_record_ids_json, []),
        metadata: safeJsonParse(row.metadata_json, {}),
        provenance: safeJsonParse(row.provenance_json, {}),
        createdAt: row.created_at,
        updatedAt: row.updated_at
...
function mapSleepSourceRecord(row) {
    return {
        id: row.id,
        importRunId: row.import_run_id,
        pairingSessionId: row.pairing_session_id,
...
function listDailySummaryRows(summaryType, userIds) {
...
function listSleepSegmentRowsBySleepId(sleepId) {
    return getDatabase()
        .prepare(`SELECT *
...
function listSleepSourceRecordRowsBySleepId(sleepId) {
    return getDatabase()
        .prepare(`SELECT *
...
function listSleepRawLogRowsBySleepId(sleepId) {
    return getDatabase()
        .prepare(`SELECT *
...
function sleepSessionDateKey(row) {
...
function inferHistoricalSleepTimeZone(values) {
...
function inferHistoricalRawStage(payload) {
    const breakdown = Array.isArray(payload.stageBreakdown)
        ? payload.stageBreakdown
        : [];
    const positiveStages = breakdown.filter((stage) => typeof stage.stage === "string" &&
        typeof stage.seconds === "number" &&
        stage.seconds > 0);
    if (positiveStages.length === 1) {
        return positiveStages[0]?.stage || "asleep_unspecified";
    }
...
function sleepHasReflection(session) {
...
function sleepStageShare(session) {
...
function buildSleepCalendarDay(session) {
    return {
        dateKey: session.localDateKey,
        sleepId: session.id,
        startedAt: session.startedAt,
...
function pickDisplaySleepSessions(sessions) {
    ...
    for (const session of sessions) {
        ...
        const shouldReplace = nextIsProviderBacked > currentIsProviderBacked ||
...
function timelineStagePriority(stage) {
    switch (stage) {
        case "awake":
            return 5;
        case "rem":
...
function buildSleepPhaseTimeline(sleep, rawSegments) {
    const sessionStartMs = Date.parse(sleep.startedAt);
    const sessionEndMs = Date.parse(sleep.endedAt);
    const totalSeconds = Math.max(1, durationSecondsBetween(sleep.startedAt, sleep.endedAt));
    ...
    const boundaries = new Set([sessionStartMs, sessionEndMs]);
    ...
    const blocks = [];
    const lastIndexByLane = new Map();
    const pushBlock = (lane, stage, startedMs, endedMs) => {
        ...
        if (lastIndex !== undefined) {
            ...
            if (previous &&
                ...
                previous.durationSeconds += Math.max(0, Math.round((endedMs - startedMs) / 1000));
                previous.widthRatio = round(Math.max(0.002, previous.durationSeconds / totalSeconds), 6);
                return;
            }
        }
    ...
    for (let index = 0; index < ordered.length - 1; index += 1) {
        ...
        const covering = segments.filter((segment) => segment.startedMs <= startedMs && segment.endedMs >= endedMs);
        if (covering.length === 0) {
            continue;
        }
        const hasInBedCoverage = covering.some((segment) => segment.normalizedStage === "in_bed");
        const sleepStage = covering
            .map((segment) => segment.normalizedStage)
            .filter((stage) => stage !== "in_bed")
...
export function getSleepSessionDetailById(sleepId) {
...
function normalizeHealthAuthorizationStatus(healthAccessAuthorized, sleepCount, workoutCount, vitalDayCount) {
...
function normalizeSleepNightInput(input) {
    const sourceTimezone = resolveTimeZone("sourceTimezone" in input ? input.sourceTimezone : "UTC");
    const localDateKey = "localDateKey" in input && typeof input.localDateKey === "string"
        ? input.localDateKey
    ...
    return {
        ...
        rawSegmentCount: "rawSegmentCount" in input && typeof input.rawSegmentCount === "number"
            ? input.rawSegmentCount
            : 0,
        stageBreakdown: input.stageBreakdown,
        recoveryMetrics: input.recoveryMetrics ?? {},
        sourceMetrics: "sourceMetrics" in input && input.sourceMetrics
            ? input.sourceMetrics
            : {
...
function normalizeSleepSegmentInput(input) {
    const sourceTimezone = resolveTimeZone(input.sourceTimezone);
    return {
        ...input,
        sourceTimezone,
...
function listNormalizedSleepSegments(payload) {
    return payload.sleepSegments.map(normalizeSleepSegmentInput);
}
function listNormalizedSleepRawRecords(payload) {
    return payload.sleepRawRecords.map(normalizeSleepRawRecordInput);
}
function refreshSleepSessionRawSegmentCount(sleepSessionId) {
...
function upsertSleepSourceRecord(input) {
    const existing = getDatabase()
       ...
       WHERE user_id = ? AND provider = 'apple_health' AND provider_record_uid = ?`)
        .get(input.pairing.user_id, input.rawRecord.externalUid);
    const now = nowIso();
    if (existing) {
        getDatabase()
            .prepare(`UPDATE health_sleep_source_records
         ...
         WHERE id = ?`)
            .run(input.importRunId, input.pairing.id, input.sleepSessionId, input.pairing.device_name ?? "", input.rawRecord.sourceTimezone, input.rawRecord.localDateKey, input.rawRecord.startedAt, input.rawRecord.endedAt, input.rawRecord.providerRecordType, input.rawRecord.rawStage, input.rawRecord.rawValue ?? null, input.qualityKind, JSON.stringify(input.rawRecord.payload ?? {}), JSON.stringify(input.rawRecord.metadata ?? {}), now, existing.id);
        return { mode: "updated", id: existing.id };
    }
    const id = `sraw_${randomUUID().replaceAll("-", "").slice(0, 12)}`;
    getDatabase()
        .prepare(`INSERT INTO health_sleep_source_records (
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(id, input.importRunId, input.pairing.id, input.sleepSessionId, input.pairing.user_id, "apple_health", input.rawRecord.providerRecordType, input.rawRecord.externalUid, input.pairing.device_name ?? "", input.rawRecord.sourceTimezone, input.rawRecord.localDateKey, input.rawRecord.startedAt, input.rawRecord.endedAt, input.rawRecord.rawStage, input.rawRecord.rawValue ?? null, input.qualityKind, JSON.stringify(input.rawRecord.payload ?? {}), JSON.stringify(input.rawRecord.metadata ?? {}), now);
...
function upsertSleepSegment(input) {
    const existing = getDatabase()
       ...
       WHERE user_id = ? AND source = 'apple_health' AND external_uid = ?`)
        .get(input.pairing.user_id, input.segment.externalUid);
    const now = nowIso();
    if (existing) {
        getDatabase()
            .prepare(`UPDATE health_sleep_segments
         ...
         WHERE id = ?`)
            .run(input.importRunId, input.pairing.id, input.sleepSessionId, "healthkit_segment", input.pairing.device_name ?? "", input.segment.sourceTimezone, input.segment.localDateKey, input.segment.startedAt, input.segment.endedAt, input.segment.stage, input.segment.bucket, input.segment.sourceValue ?? null, input.qualityKind ?? "provider_native", JSON.stringify(input.sourceRecordIds ?? []), JSON.stringify(input.segment.metadata ?? {}), JSON.stringify({
    ...
    getDatabase()
        .prepare(`INSERT INTO health_sleep_segments (
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(id, input.segment.externalUid, input.importRunId, input.pairing.id, input.sleepSessionId, input.pairing.user_id, "apple_health", "healthkit_segment", input.pairing.device_name ?? "", input.segment.sourceTimezone, input.segment.localDateKey, input.segment.startedAt, input.segment.endedAt, input.segment.stage, input.segment.bucket, input.segment.sourceValue ?? null, input.qualityKind ?? "provider_native", JSON.stringify(input.sourceRecordIds ?? []), JSON.stringify(input.segment.metadata ?? {}), JSON.stringify({
        importedVia: "ios_companion",
        importRunId: input.importRunId,
        createdAt: now
    }), now, now);
...
function createSleepRawLog(input) {
    ...
    getDatabase()
        .prepare(`INSERT INTO health_sleep_raw_logs (
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(id, input.importRunId, input.pairingSessionId, input.sleepSessionId, input.userId, input.source, input.logType, input.externalUid ?? null, resolveTimeZone(input.sourceTimezone), input.localDateKey ?? "", input.startedAt ?? null, input.endedAt ?? null, JSON.stringify(input.payload), JSON.stringify(input.metadata ?? {}), now);
}
function syncPairingSourceStatesFromPayload(pairing, payload) {
    upsertPairingSourceState(pairing, "health", {
        ...
        authorizationStatus: normalizeHealthAuthorizationStatus(payload.permissions.healthKitAuthorized, Math.max(payload.sleepNights.length, payload.sleepSessions.length), payload.workouts.length, payload.vitals.daySummaries.length),
...
function normalizeVitalMetricSummary(input) {
    return {
        metric: input.metric,
        label: input.label,
...
function upsertVitalDaySummary(userId, input) {
    const metrics = input.metrics.reduce((accumulator, metric) => {
...
function insertOrUpdateSleepSession(pairing, input) {
    ...
    if (existing) {
        getDatabase()
         ...
         WHERE id = ?`)
            .run(pairing.id, pairing.device_name ?? "", input.sourceTimezone, input.localDateKey, input.startedAt, input.endedAt, input.timeInBedSeconds, input.asleepSeconds, input.awakeSeconds, sleepScore, timingMetrics.regularityScore, timingMetrics.bedtimeConsistencyMinutes, timingMetrics.wakeConsistencyMinutes, input.rawSegmentCount, JSON.stringify(stageBreakdown), JSON.stringify(input.recoveryMetrics), JSON.stringify(input.sourceMetrics), JSON.stringify(mergeHealthLinks(safeJsonParse(existing.links_json, []), input.links, annotations.links)), JSON.stringify(annotations), JSON.stringify({
            importedVia: "ios_companion",
    ...
    getDatabase()
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(id, input.externalUid, pairing.id, pairing.user_id, "apple_health", "healthkit", pairing.device_name ?? "", input.sourceTimezone, input.localDateKey, input.startedAt, input.endedAt, input.timeInBedSeconds, input.asleepSeconds, input.awakeSeconds, sleepScore, timingMetrics.regularityScore, timingMetrics.bedtimeConsistencyMinutes, timingMetrics.wakeConsistencyMinutes, input.rawSegmentCount, JSON.stringify(stageBreakdown), JSON.stringify(input.recoveryMetrics), JSON.stringify(input.sourceMetrics), JSON.stringify(mergeHealthLinks(input.links, annotations.links)), JSON.stringify(annotations), JSON.stringify({
        importedVia: "ios_companion",
        pairingSessionId: pairing.id,
        createdAt: now
...
function selectAuthoritativeSleepRows(rows) {
    const authoritativeRows = rows.filter((candidate) => {
        const containsOther = rows.some((other) => {
            if (other.id === candidate.id) {
                return false;
            }
            return (Date.parse(candidate.started_at) <= Date.parse(other.started_at) &&
...
function backfillHistoricalSleepEvidence() {
    ...
    const updateSessionTimezoneStatement = getDatabase().prepare(`UPDATE health_sleep_sessions
     SET source_timezone = ?, local_date_key = ?, updated_at = ?
     WHERE id = ?`);
    const detachSourceRecordsStatement = getDatabase().prepare(`UPDATE health_sleep_source_records
     SET sleep_session_id = NULL, metadata_json = ?
     WHERE sleep_session_id = ?`);
    const detachSegmentsStatement = getDatabase().prepare(`UPDATE health_sleep_segments
     SET sleep_session_id = NULL, provenance_json = ?, updated_at = ?
     WHERE sleep_session_id = ?`);
    const detachRawLogsStatement = getDatabase().prepare(`UPDATE health_sleep_raw_logs
     SET sleep_session_id = NULL, metadata_json = ?
     WHERE sleep_session_id = ?`);
    const deleteSessionStatement = getDatabase().prepare(`DELETE FROM health_sleep_sessions WHERE id = ?`);
    for (const session of repairedSessions) {
        ...
        if (session.source_timezone !== sourceTimezone ||
        ...
        if (existingSegmentCount.count === 0 && rawLogs.length > 0) {
            ...
            for (const rawLog of authoritativeLogs) {
             ...
             LIMIT 1`)
                    .get(session.id, rawLog.external_uid ?? rawLog.id);
                getDatabase()
                    .prepare(`INSERT INTO health_sleep_segments (
               id, external_uid, import_run_id, pairing_session_id, sleep_session_id, user_id, source, source_type,
               source_device, source_timezone, local_date_key, started_at, ended_at, stage, bucket, source_value,
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                    .run(`sleepseg_${randomUUID().replaceAll("-", "").slice(0, 12)}`, rawLog.external_uid ?? rawLog.id, rawLog.import_run_id, rawLog.pairing_session_id, session.id, rawLog.user_id, "apple_health", "historical_import_segment", session.source_device, sourceTimezone, localDateKey, rawLog.started_at, rawLog.ended_at, inferredStage, inferHistoricalSleepBucket(inferredStage), null, "historical_import", JSON.stringify(sourceRecord ? [sourceRecord.id] : []), JSON.stringify({
                    historicalImport: true
                }), JSON.stringify({
                    backfilledFrom: "health_sleep_raw_logs",
                    rawLogId: rawLog.id
                }), rawLog.created_at, nowIso());
            }
        ...
        if (providerBackedSourceRecordCount.count > 0) {
           ...
           WHERE user_id = ?
             ...
             AND source_type = 'healthkit_repaired'
             AND id != ?`)
                .all(session.user_id, localDateKey, session.id);
            for (const historical of competingHistoricalSessions) {
                const mergedAnnotations = mergeSleepAnnotationPayloads([
                    safeJsonParse(session.annotations_json, {}),
                    safeJsonParse(historical.annotations_json, {})
                ], [
                    safeJsonParse(session.links_json, []),
                    safeJsonParse(historical.links_json, [])
                ]);
                getDatabase()
                    .prepare(`UPDATE health_sleep_sessions
             SET links_json = ?, annotations_json = ?, updated_at = ?
             WHERE id = ?`)
                    .run(JSON.stringify(mergedAnnotations.links), JSON.stringify(mergedAnnotations), nowIso(), session.id);
                detachSourceRecordsStatement.run(JSON.stringify({
                    replacedBySleepSessionId: session.id,
                    replacedAt: nowIso()
                }), historical.id);
                detachSegmentsStatement.run(JSON.stringify({
                    replacedBySleepSessionId: session.id,
                    replacedAt: nowIso()
                }), nowIso(), historical.id);
                detachRawLogsStatement.run(JSON.stringify({
                    replacedBySleepSessionId: session.id,
                    replacedAt: nowIso()
                }), historical.id);
        ...
        if (legacyRows.length > 0) {
            for (const row of legacyRows) {
             ...
             WHERE source = 'apple_health'
               ...
               AND external_uid = ?
             LIMIT 1`)
                    .get(row.user_id, row.external_uid);
                if (!rawLogExists) {
                    createSleepRawLog({
                        sleepSessionId: null,
                        importRunId: null,
            ...
            for (const [userId, rows] of byUser.entries()) {
                ...
                for (const cluster of clusters) {
                    ...
                    const timeInBedSeconds = Math.max(durationSecondsBetween(startedAt, endedAt), asleepSeconds);
                    const awakeSeconds = Math.max(0, timeInBedSeconds - asleepSeconds);
                    const stageTotals = new Map();
                    for (const row of rowsForNight) {
                        for (const stage of safeJsonParse(row.stage_breakdown_json, [])) {
                            if (stage.seconds <= 0) {
                                continue;
                            }
                    ...
                    const annotations = mergeSleepAnnotationPayloads(cluster.map((row) => safeJsonParse(row.annotations_json, {})), cluster.map((row) => safeJsonParse(row.links_json, [])));
                    const recoveryMetrics = cluster.reduce((acc, row) => {
                        Object.assign(acc, safeJsonParse(row.recovery_metrics_json, {}));
                        return acc;
                    }, {});
                    const sourceMetrics = {
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                        .run(id, `legacy-repair-${startedAt}-${endedAt}`, cluster[0]?.pairing_session_id ?? null, userId, "apple_health", "healthkit_repaired", cluster[0]?.source_device ?? "", sourceTimezone, localDateKey, startedAt, endedAt, timeInBedSeconds, asleepSeconds, awakeSeconds, sleepScore, timingMetrics.regularityScore, timingMetrics.bedtimeConsistencyMinutes, timingMetrics.wakeConsistencyMinutes, rowsForNight.length, JSON.stringify(stageBreakdown), JSON.stringify(recoveryMetrics), JSON.stringify(sourceMetrics), JSON.stringify(annotations.links), JSON.stringify(annotations), JSON.stringify({
                        repairedFromLegacy: true,
                        legacyRowIds: cluster.map((row) => row.id)
                    }), JSON.stringify(derived), nowIso(), nowIso());
                    const legacyExternalUids = cluster
                        .map((row) => row.external_uid)
                        .filter((value) => value.trim().length > 0);
                    if (legacyExternalUids.length > 0) {
                        getDatabase()
                            .prepare(`UPDATE health_sleep_raw_logs
            ...
            const deleteStatement = getDatabase().prepare(`DELETE FROM health_sleep_sessions
         WHERE id = ?`);
            for (const row of legacyRows) {
...
function replaceHistoricalSleepSessionsForDate(userId, localDateKey, providerBackedSleepSessionId) {
    const providerRecordCount = getDatabase()
        .prepare(`SELECT COUNT(*) AS count
       FROM health_sleep_source_records
       WHERE sleep_session_id = ? AND quality_kind = 'provider_native'`)
        .get(providerBackedSleepSessionId);
    ...
    const historicalSessions = getDatabase()
       ...
       WHERE user_id = ?
         ...
         AND source_type = 'healthkit_repaired'
         AND id != ?`)
        .all(userId, localDateKey, providerBackedSleepSessionId);
    ...
    for (const historical of historicalSessions) {
        const currentProviderSession = getDatabase()
            ...
            .get(providerBackedSleepSessionId);
        const mergedAnnotations = mergeSleepAnnotationPayloads([
            safeJsonParse(currentProviderSession.annotations_json, {}),
            safeJsonParse(historical.annotations_json, {})
        ], [
            safeJsonParse(currentProviderSession.links_json, []),
            safeJsonParse(historical.links_json, [])
        ]);
        getDatabase()
            .prepare(`UPDATE health_sleep_sessions
         SET links_json = ?, annotations_json = ?, updated_at = ?
         WHERE id = ?`)
            .run(JSON.stringify(mergedAnnotations.links), JSON.stringify(mergedAnnotations), nowIso(), providerBackedSleepSessionId);
        getDatabase()
            .prepare(`UPDATE health_sleep_source_records
         SET sleep_session_id = NULL, metadata_json = ?
         WHERE sleep_session_id = ?`)
...
function markMobileHealthSyncChunkApplied(input) {
    const now = nowIso();
    getDatabase()
        .prepare(`UPDATE health_mobile_sync_chunks
...
function applyMovementChunkImmediately(session, payload) {
    if (!payload.movement) {
        return false;
    }
    const pairing = mobileSyncSessionPairing(session);
...
function applyScreenTimeChunkImmediately(session, payload) {
    if (!payload.screenTime) {
        return false;
    }
    const pairing = mobileSyncSessionPairing(session);
    const metadata = mobileSyncSessionMetadata(session);
    ingestScreenTimeSync(pairing, payload.screenTime, metadata.device?.sourceDevice ?? pairing.device_name ?? "iPhone");
    return true;
}
function applyVitalsChunkImmediately(session, payload) {
    if (!payload.vitals) {
        return false;
    }
    const pairing = mobileSyncSessionPairing(session);
...
function applyMobileHealthSyncChunkImmediately(session, family, payload) {
    switch (family) {
        case "workout_summaries":
        case "workout_archive":
...
export function ingestMobileHealthSyncChunk(syncSessionId, payload, rawPayloadJson) {
    ...
    return runInTransaction(() => {
        ...
        getDatabase()
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, ?)`)
            .run(mobileSyncChunkRecordId(), syncSessionId, parsed.chunkId, parsed.sequence, parsed.family, serverChecksum, parsed.recordCount, actualByteCount, payloadJson, JSON.stringify(payloadSummary), now, now, now);
        const appliedMode = applyMobileHealthSyncChunkImmediately(session, parsed.family, wirePayload.payload);
        if (appliedMode) {
...
function mergeMobileHealthSyncChunks(session, chunks, options = {}) {
    const pairing = getDatabase()
        .prepare(`SELECT * FROM companion_pairing_sessions WHERE id = ?`)
        .get(session.pairing_session_id);
    if (!pairing) {
...
function aggregateMobileSyncChunkCounts(chunks) {
    ...
    for (const chunk of chunks) {
        const summary = safeJsonParse(chunk.payload_summary_json, {});
        const numberValue = (key) => typeof summary[key] === "number" && Number.isFinite(summary[key])
            ? summary[key]
            : 0;
        switch (chunk.family) {
            case "sleep_nights":
...
function syncReceiptWithChunkCounts(sync, chunks) {
    ...
    return {
        ...
        imported: {
            ...
            sleepSegments: Math.max(sync.imported.sleepSegments ?? 0, counts.sleepSegments),
            sleepRawRecords: Math.max(sync.imported.sleepRawRecords ?? 0, counts.sleepRawRecords),
            workouts: Math.max(sync.imported.workouts, counts.workouts),
            vitalsDaySummaries: Math.max(sync.imported.vitalsDaySummaries ?? 0, counts.vitalsDaySummaries),
            movementKnownPlaces: Math.max(sync.imported.movementKnownPlaces ?? 0, counts.movementKnownPlaces),
            movementStays: Math.max(sync.imported.movementStays ?? 0, counts.movementStays),
            movementTrips: Math.max(sync.imported.movementTrips ?? 0, counts.movementTrips),
            screenTimeDaySummaries: Math.max(sync.imported.screenTimeDaySummaries ?? 0, counts.screenTimeDaySummaries),
            screenTimeHourlySegments: Math.max(sync.imported.screenTimeHourlySegments ?? 0, counts.screenTimeHourlySegments)
        }
    };
}
...
export function ingestMobileHealthSync(payload) {
    ...
    return runInTransaction(() => {
        ...
        const vitalMetricEntries = parsed.vitals.daySummaries.reduce((sum, day) => sum + day.metrics.length, 0);
        let createdCount = 0;
        let updatedCount = 0;
        let mergedCount = 0;
        getDatabase()
        ...
        for (const rawRecord of normalizedSleepRawRecords) {
            ...
            const sleepSessionId = candidateSessions.find((session) => Date.parse(rawRecord.endedAt) > Date.parse(session.startedAt) &&
                Date.parse(rawRecord.startedAt) < Date.parse(session.endedAt))?.id ?? null;
        ...
        for (const segment of normalizedSleepSegments) {
            ...
            const sleepSessionId = candidateSessions.find((session) => Date.parse(segment.endedAt) > Date.parse(session.startedAt) &&
                Date.parse(segment.startedAt) < Date.parse(session.endedAt))?.id ?? null;
        ...
        getDatabase()
         ...
         SET source_device = ?, status = 'completed', payload_summary_json = ?,
             ...
             imported_at = ?, updated_at = ?
         WHERE id = ?`)
            .run(parsed.device.sourceDevice, JSON.stringify({
            permissions: parsed.permissions,
            ...
            parsed.screenTime.hourlySegments.length, createdCount + movementSync.createdCount + screenTimeSync.createdCount, updatedCount + movementSync.updatedCount + screenTimeSync.updatedCount, mergedCount, now, now, runId);
        recordActivityEvent({
            entityType: "system",
            entityId: pairing.id,
...
export function getCompanionOverview(userIds) {
    ...
    const sleepSegments = getDatabase()
        .prepare(userIds && userIds.length > 0
...
export function getVitalsViewData(userIds) {
    ...
    const metrics = [...metricBuckets.entries()]
        ...
        const recentValues = days
            .map((day) => vitalMetricPrimaryValue({
            aggregation: bucket.aggregation,
            latest: day.latest,
            average: day.average,
            total: day.total,
...
export function updateSleepSession(sleepId, patch, activity) {
    ...
    const sourceMetrics = parsed.sourceMetrics ?? safeJsonParse(current.source_metrics_json, {});
    ...
    getDatabase()
       ...
       SET external_uid = ?, source = ?, source_type = ?, source_device = ?, source_timezone = ?, local_date_key = ?,
           ...
           provenance_json = ?, derived_json = ?, updated_at = ?
       WHERE id = ?`)
        .run(parsed.externalUid ?? current.external_uid, parsed.source ?? current.source, parsed.sourceType ?? current.source_type, parsed.sourceDevice ?? current.source_device, sourceTimezone, (parsed.localDateKey ?? current.local_date_key) ||
        localDateKeyForTimezone(endedAt, sourceTimezone), startedAt, endedAt, timeInBedSeconds, asleepSeconds, awakeSeconds, sleepScore, timingMetrics.regularityScore, timingMetrics.bedtimeConsistencyMinutes, timingMetrics.wakeConsistencyMinutes, parsed.rawSegmentCount ?? current.raw_segment_count, JSON.stringify(stageBreakdown), JSON.stringify(recoveryMetrics), JSON.stringify(sourceMetrics), JSON.stringify(links), JSON.stringify(annotations), JSON.stringify({
        ...currentProvenance,
        ...(parsed.provenance ?? {}),
        updatedAt: now,
    ...
    summarizeUserHealthDay(current.user_id, (parsed.localDateKey ?? current.local_date_key) ||
