...
function buildStoredEnergyModel(input) {
    ...
    const dailySummaryRows = getDatabase()
       ...
       ORDER BY date_key DESC`)
        .all(input.userId, startKey, todayKey);
    ...
    const todayMovementCaloriesFromRange = input.dayStartAt && input.dayEndAt
        ? getDatabase()
            ...
            .get(input.userId, input.dayStartAt, input.dayEndAt)
        : null;
    const activeBaselineHealthKitDays = dailyHealthKit.filter((day) => day.dateKey < todayKey && day.activeEnergyKcal != null);
    const activeEnergyAverage = average(activeBaselineHealthKitDays.map((day) => day.activeEnergyKcal));
    ...
    const workoutBaselineEntries = [...workoutByDay.entries()].filter(([dateKey, value]) => dateKey < todayKey && value != null);
    const movementBaselineEntries = [...movementByDay.entries()].filter(([dateKey, value]) => dateKey < todayKey && value != null);
    ...
    const fallbackActiveBurn = workoutEnergyAverage != null || movementCaloriesAverage != null
        ...
        : null;
    const activeBurnKcal = activeEnergyAverage ?? fallbackActiveBurn ?? input.defaultActiveCalories;
    ...
    return {
        ...
        exerciseMinutesAverage: average(dailyHealthKit
            .filter((day) => day.dateKey < todayKey)
            .map((day) => day.exerciseMinutes)),
        stepCountAverage: average(dailyHealthKit
            .filter((day) => day.dateKey < todayKey)
