...
import { buildNotesSummaryByEntity, createNote, getNoteById, listNotesByObservedAtRange, listNotes, updateNote } from "./repositories/notes.js";
...
function buildPlaybookRouteInfo(focus) {
    ...
    return {
        routePosture,
        apiAccessHint: apiAccessHint || "Route posture is not published for this playbook focus."
    };
}
function enrichConversationPlaybookWithRouteInfo(playbook) {
    return {
        ...playbook,
        ...
        entityRouteModel: {
            ...
            actionEntities: {
                ...
                work_adjustment: {
                    ...
                    writeModel: "Apply a signed minute correction to an existing task or project. Use this instead of creating a fake task_run for work that already happened."
                },
                questionnaire_run: {
...
function readNullableStringField(record, key) {
    const value = record[key];
...
function compactProject(project) {
    return {
        id: project.id,
        title: project.title,
        summary: compactText(project.description || project.productRequirementsDocument),
...
function compactNote(note) {
    ...
    return {
        ...
        preview: compactText(note.summary || note.contentPlain || note.contentMarkdown, NOTE_PREVIEW_LIMIT),
        kind: note.kind,
...
function compactNamedRecord(value, options = {}) {
...
function compactOperatorContext(context) {
    return {
        generatedAt: context.generatedAt,
        ...
        currentBoard: Object.fromEntries(Object.entries(context.currentBoard).map(([lane, tasks]) => [
            lane,
            {
                count: tasks.length,
...
function compactFitness(fitness) {
    return {
...
function compactWeightLoss(weightLoss) {
    return {
        ...
        energyModel: {
            ...
            baselineActiveCaloriesKcal: weightLoss.energyModel.baselineActiveCaloriesKcal,
            todayActiveCaloriesKcal: weightLoss.energyModel.todayActiveCaloriesKcal,
            todayActiveCaloriesSource: weightLoss.energyModel.todayActiveCaloriesSource,
            todayTargetAdjustmentKcal: weightLoss.energyModel.todayTargetAdjustmentKcal,
...
function compactPsyche(psyche) {
    ...
    return {
        ...
        values: psyche.values.slice(0, 8).map((value) => compactNamedRecord(value, {
            summaryFields: ["summary", "description", "whyItMatters"]
        })),
        patterns: psyche.patterns.slice(0, 8).map((pattern) => compactNamedRecord(pattern, {
            summaryFields: ["loopSummary", "longTermCost", "preferredResponse"]
        })),
        behaviors: psyche.behaviors.slice(0, 8).map((behavior) => compactNamedRecord(behavior, {
            summaryFields: ["description", "function", "cost", "alternative"]
        })),
        beliefs: psyche.beliefs.slice(0, 8).map((belief) => compactNamedRecord(belief, {
            summaryFields: ["statement", "evidenceFor", "alternative"]
        })),
        modes: psyche.modes.slice(0, 8).map((mode) => compactNamedRecord(mode, {
            summaryFields: ["description", "fear", "burden", "protectiveJob"]
        })),
        recentReports: psyche.reports.slice(0, 5).map((report) => compactNamedRecord(report, {
...
function compactCalendarItem(item) {
    const record = toRecord(item);
    return {
        id: readStringField(record, "id"),
        title: compactText(readStringField(record, "title"), 100),
        startAt: readStringField(record, "startAt") || readStringField(record, "startsAt"),
...
function compactCalendarForDays(calendar, todayKey, yesterdayKey) {
    const itemDateKey = (item) => {
        const record = toRecord(item);
        const startAt = readStringField(record, "startAt") || readStringField(record, "startsAt");
...
function compactOnboardingPayload(onboarding) {
    return {
        ...
        specializedDomainRoutes: onboarding.entityRouteModel.specializedDomainSurfaces,
...
function buildSignalMatrix(input) {
    return [
        ...
        {
            lane: "today",
            signal: input.today.directive.task?.title ??
...
function buildOperatorOverview(request) {
    ...
    if (requestedDetailMode !== "compact") {
        warnings.push("Operator overview always returns the compact progressive payload. Use drill-down routes such as /api/v1/context, /api/v1/operator/context, /api/v1/notes/:id, or /api/v1/entities/search for full records.");
    }
    const now = new Date();
    ...
    const calendar = compactCalendarForDays(readCalendarOverview({
        ...
        to: todayRange.to,
        userIds
    }), todayRange.dateKey, yesterdayRange.dateKey);
    ...
    return {
        generatedAt: now.toISOString(),
        detailMode: "compact",
        summary: "Compact operator overview for agent continuity. It keeps current direction, today/yesterday, health, calendar, psyche, notes, and IDs, while full records stay behind drill-down routes.",
        ...
        snapshot: {
            ...
            userScope: {
                ...
                selectedUsers: userIds === undefined
                    ? users
...
<html lang="en">
  ...
  <body>
    ...
    app.get("/api/v1/metrics", async (request) => {
        const userIds = resolveScopedUserIds(request.query);
        return {
            metrics: buildGamificationOverview(filterOwnedEntities("goal", listGoals(), userIds), filterOwnedEntities("task", listTasks({ userIds }), userIds), filterOwnedEntities("habit", listHabits(), userIds), new Date(), { userIds })
        };
    });
