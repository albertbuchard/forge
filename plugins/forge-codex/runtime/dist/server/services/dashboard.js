...
function dueDateWeight(task) {
    return task.dueDate ? Date.parse(`${task.dueDate}T00:00:00.000Z`) : Number.POSITIVE_INFINITY;
}
function takeExecutionSlice(tasks, limit = 4) {
    return tasks.slice(0, limit);
}
function buildExecutionBuckets(tasks, todayIso, weekEndIso) {
...
function buildGoalSummary(tasks, goalId) {
...
export function getDashboard(options = {}) {
    ...
    const completedThisWeek = tasks.filter((task) => task.completedAt !== null && task.completedAt >= weekStart).length;
    const totalPoints = tasks
        .filter((task) => task.status === "done")
        .reduce((sum, task) => sum + task.points, 0);
    const focusTasks = tasks.filter((task) => task.status === "focus" || task.status === "in_progress").length;
    const alignedCompletedTasks = tasks.filter((task) => task.status === "done" && task.goalId !== null && task.tagIds.length > 0).length;
    const overdueTasks = tasks.filter((task) => task.status !== "done" && task.dueDate !== null && task.dueDate < todayIso).length;
    const dueThisWeek = tasks.filter((task) => task.status !== "done" && task.dueDate !== null && task.dueDate >= todayIso && task.dueDate <= weekEndIso).length;
