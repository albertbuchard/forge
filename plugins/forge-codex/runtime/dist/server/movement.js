...
function maybeCreateStayNote(settings, stay, place) {
...
function maybeCreateTripNote(settings, trip, startPlace, endPlace) {
    if (!settings || settings.publishMode === "no_publish") {
        return null;
    }
...
function replaceTripChildren(tripId, points, stops, userId) {
    getDatabase()
    ...
    points.forEach((point, index) => {
        pointInsert.run(`mtp_${randomUUID().replaceAll("-", "").slice(0, 10)}`, tripId, index, point.recordedAt, point.latitude, point.longitude, point.accuracyMeters, point.altitudeMeters, point.speedMps, point.isStopAnchor ? 1 : 0, now);
    });
...
function upsertMovementTrip(pairing, settings, input) {
    ...
    const effectiveExpectedMet = parsed.expectedMet ?? inferExpectedMet(parsed.activityType, parsed.averageSpeedMps);
    ...
    getDatabase()
       ...
       ON CONFLICT(user_id, external_uid) DO UPDATE SET
         ...
         metadata_json = excluded.metadata_json,
         updated_at = excluded.updated_at`)
        .run(id, parsed.externalUid, pairing.id, pairing.user_id, startPlace?.id ?? null, endPlace?.id ?? null, parsed.label, parsed.status, parsed.travelMode, parsed.activityType, parsed.startedAt, parsed.endedAt, parsed.distanceMeters, parsed.movingSeconds, parsed.idleSeconds, parsed.averageSpeedMps, parsed.maxSpeedMps, parsed.caloriesKcal, effectiveExpectedMet, JSON.stringify({}), JSON.stringify(uniqStrings(parsed.tags)), JSON.stringify(parsed.linkedEntities), JSON.stringify(parsed.linkedPeople), JSON.stringify(parsed.metadata), existing?.published_note_id ?? null, existing?.created_at ?? now, now);
    const fresh = getDatabase()
