...
export const forgeThemeCatalog = {
    obsidian: {
        label: "Obsidian",
...
export const forgeThemeOptions = [
    {
        value: "obsidian",
...
function mixHex(base, overlay, weight) {
...
export function getForgeThemePreview(preference, customTheme) {
...
export function buildForgeThemeVariables(theme) {
    ...
    return {
        ...
        "--surface-psyche-high": alpha(mixHex(theme.panelHigh, theme.secondary, 0.1), 0.96),
        "--hero-gradient": `linear-gradient(180deg, ${alpha(heroTop, 0.95)}, ${alpha(heroBottom, 0.95)})`,
        "--card-gradient": `linear-gradient(180deg, ${alpha(cardTop, 0.96)}, ${alpha(theme.panelLow, 0.94)})`,
        ...
        "--ui-ink": theme.ink,
        "--ui-ink-muted": alpha(theme.ink, 0.58),
...
export function applyForgeThemeToDocument(preference, customTheme) {
