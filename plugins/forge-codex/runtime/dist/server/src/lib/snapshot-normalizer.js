...
function normalizeGoal(goal) {
    return {
        ...
        horizon: goal?.horizon === "quarter" || goal?.horizon === "lifetime"
            ? goal.horizon
            : "year",
        status: goal?.status === "paused" || goal?.status === "completed"
            ? goal.status
            : "active",
...
function normalizeTag(tag) {
    return {
        id: tag?.id ?? "",
        name: tag?.name ?? "",
        kind: tag?.kind === "value" || tag?.kind === "execution"
            ? tag.kind
            : "category",
...
function normalizeStrategy(strategy) {
    return {
        ...
        status: strategy?.status === "paused" || strategy?.status === "completed"
            ? strategy.status
            : "active",
        ...
        lockedByUser: strategy?.lockedByUser ?? null,
...
export function normalizeForgeSnapshot(raw) {
    ...
    const dashboardProjects = (legacy.dashboard?.projects ??
        ...
        rootProjects).map(normalizeProject);
    const overviewProjects = (legacy.overview?.projects ??
        legacy.overview?.campaigns ??
        dashboardProjects).map(normalizeProject);
    return {
        ...raw,
        meta: {
            apiVersion: "v1",
        ...
        metrics: {
            ...
            xpToNextLevel: raw.metrics?.xpToNextLevel ?? raw.metrics?.nextLevelXp ?? 120,
            currentLevelStartXp: raw.metrics?.currentLevelStartXp ?? 0,
            nextLevelTotalXp: raw.metrics?.nextLevelTotalXp ??
        ...
        overview: {
            ...
            strategicHeader: {
                streakDays: raw.overview?.strategicHeader?.streakDays ??
                ...
                currentLevelXp: raw.overview?.strategicHeader?.currentLevelXp ??
                    raw.metrics?.currentLevelXp ??
                    0,
                nextLevelXp: raw.overview?.strategicHeader?.nextLevelXp ??
                    raw.metrics?.nextLevelXp ??
                    120,
                momentumScore: raw.overview?.strategicHeader?.momentumScore ??
