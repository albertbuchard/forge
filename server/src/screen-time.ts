...
export const screenTimeSettingsInputSchema = z.object({
  ...
  authorizationStatus: screenTimeAuthorizationStatusSchema.default(
    "not_determined"
  ),
...
type ScreenTimeSettingsRow = {
  user_id: string;
  tracking_enabled: number;
  sync_enabled: number;
  authorization_status: z.infer<typeof screenTimeAuthorizationStatusSchema>;
  capture_state: z.infer<typeof screenTimeCaptureStateSchema>;
  last_captured_day_key: string | null;
  last_capture_started_at: string | null;
  last_capture_ended_at: string | null;
  metadata_json: string;
  created_at: string;
  updated_at: string;
};

type ScreenTimeDaySummaryRow = {
  id: string;
  user_id: string;
  ...
  longest_activity_seconds: number;
  top_app_bundle_ids_json: string;
  top_category_labels_json: string;
  metadata_json: string;
  created_at: string;
  updated_at: string;
};

type ScreenTimeHourlySegmentRow = {
  id: string;
  user_id: string;
  pairing_session_id: string | null;
  source_device: string;
  ...
  notification_count: number;
  first_pickup_at: string | null;
  longest_activity_started_at: string | null;
  longest_activity_ended_at: string | null;
  metadata_json: string;
  created_at: string;
  updated_at: string;
};

type ScreenTimeAppUsageRow = {
  id: string;
  segment_id: string;
...
type ScreenTimeCategoryUsageRow = {
  id: string;
  segment_id: string;
...
function screenTimeCaptureStats(input: {
  authorizationStatus: z.infer<typeof screenTimeAuthorizationStatusSchema>;
  ...
  const captureAgeHours = input.lastCaptureEndedAt
    ...
    : null;
  const captureWindowDays =
    input.lastCaptureStartedAt && input.lastCaptureEndedAt
      ? Math.max(
          1,
          Math.round(
            hoursBetween(input.lastCaptureEndedAt, input.lastCaptureStartedAt) / 24
          )
        )
      : hasCapture
        ? 1
...
function buildUserFilterClause(userIds?: string[]) {
  const effectiveUserIds =
    userIds && userIds.length > 0 ? userIds : [getDefaultUser().id];
  return {
    effectiveUserIds,
...
function ensureScreenTimeSettings(userId: string) {
  const existing = getDatabase()
    ...
    .get(userId) as ScreenTimeSettingsRow | undefined;
  if (existing) {
    return existing;
  }
  const now = nowIso();
  ...
  return getDatabase()
    ...
    .get(userId) as ScreenTimeSettingsRow;
}

function mapScreenTimeSettings(row: ScreenTimeSettingsRow) {
  return {
    userId: row.user_id,
...
function upsertScreenTimeSettings(
  userId: string,
  input: z.infer<typeof screenTimeSettingsInputSchema>
) {
  const now = nowIso();
  getDatabase()
    .prepare(
    ...
    .run(
      ...
      input.captureState,
      input.lastCapturedDayKey,
      input.lastCaptureStartedAt,
      input.lastCaptureEndedAt,
...
function listHourlySegmentRows(input: {
  ...
  const params: Array<string | number> = [...effectiveUserIds];
  if (input.dateKey) {
  ...
  return getDatabase()
    ...
    .all(...params) as ScreenTimeHourlySegmentRow[];
}

function listDaySummaryRows(input: { userIds?: string[]; dateKey?: string; monthKey?: string }) {
  const { effectiveUserIds, placeholders } = buildUserFilterClause(input.userIds);
  const conditions = [`user_id IN (${placeholders})`];
  const params: Array<string | number> = [...effectiveUserIds];
  if (input.dateKey) {
  ...
  return getDatabase()
    ...
    .all(...params) as ScreenTimeDaySummaryRow[];
}

function listAppUsageRows(segmentIds: string[]) {
  if (segmentIds.length === 0) {
    return [] as ScreenTimeAppUsageRow[];
  }
  ...
  return getDatabase()
    ...
    .all(...segmentIds) as ScreenTimeAppUsageRow[];
}

function listCategoryUsageRows(segmentIds: string[]) {
  if (segmentIds.length === 0) {
    return [] as ScreenTimeCategoryUsageRow[];
  }
  const placeholders = segmentIds.map(() => "?").join(", ");
  return getDatabase()
    .prepare(
    ...
    .all(...segmentIds) as ScreenTimeCategoryUsageRow[];
}

function mapAppUsage(row: ScreenTimeAppUsageRow) {
  return {
    id: row.id,
...
function mapCategoryUsage(row: ScreenTimeCategoryUsageRow) {
  return {
    id: row.id,
    categoryLabel: row.category_label,
    totalActivitySeconds: row.total_activity_seconds
...
function mapHourlySegment(
  row: ScreenTimeHourlySegmentRow,
  apps: ScreenTimeAppUsageRow[],
  categories: ScreenTimeCategoryUsageRow[]
) {
  return {
    id: row.id,
    userId: row.user_id,
...
function aggregateAppUsage(
  ...
  return [...byBundle.values()].sort(
    (left, right) =>
      right.totalActivitySeconds - left.totalActivitySeconds ||
      right.pickupCount - left.pickupCount ||
      left.displayName.localeCompare(right.displayName)
  );
}

function aggregateCategoryUsage(rows: Array<{ categoryLabel: string; totalActivitySeconds: number }>) {
  const byCategory = new Map<string, { id: string; categoryLabel: string; totalActivitySeconds: number }>();
  rows.forEach((row) => {
...
function upsertDaySummary(
  ...
  sourceDevice: string,
  summary: z.infer<typeof screenTimeDaySummaryInputSchema>
) {
  const existing = getDatabase()
    .prepare(
      `SELECT id
    ...
    .get(pairing.user_id, sourceDevice, summary.dateKey) as { id: string } | undefined;
  const id = existing?.id ?? `std_${randomUUID().replaceAll("-", "").slice(0, 10)}`;
  const now = nowIso();
  getDatabase()
    .prepare(
      `INSERT INTO screen_time_day_summaries (
    ...
    .run(
      ...
      sourceDevice,
      summary.dateKey,
      summary.totalActivitySeconds,
      summary.pickupCount,
      summary.notificationCount,
      summary.firstPickupAt,
      summary.longestActivitySeconds,
      JSON.stringify(summary.topAppBundleIdentifiers),
      JSON.stringify(summary.topCategoryLabels),
...
function replaceSegmentChildren(
  segmentId: string,
  apps: z.infer<typeof screenTimeAppUsageInputSchema>[],
  categories: z.infer<typeof screenTimeCategoryUsageInputSchema>[]
) {
  getDatabase()
  ...
  getDatabase()
    ...
    .run(segmentId);
  const now = nowIso();
  const insertApp = getDatabase().prepare(
    `INSERT INTO screen_time_app_usage (
  ...
  apps.forEach((app) => {
    insertApp.run(
      `sta_${randomUUID().replaceAll("-", "").slice(0, 10)}`,
      segmentId,
      app.bundleIdentifier,
      app.displayName,
      app.categoryLabel,
      app.totalActivitySeconds,
      app.pickupCount,
  ...
  const insertCategory = getDatabase().prepare(
    `INSERT INTO screen_time_category_usage (
       id, segment_id, category_label, total_activity_seconds, created_at, updated_at
     ) VALUES (?, ?, ?, ?, ?, ?)`
  );
  categories.forEach((category) => {
    insertCategory.run(
      `stc_${randomUUID().replaceAll("-", "").slice(0, 10)}`,
      segmentId,
      category.categoryLabel,
      category.totalActivitySeconds,
      now,
      now
    );
  });
...
function upsertHourlySegment(
  pairing: PairingSessionLike,
  sourceDevice: string,
  segment: z.infer<typeof screenTimeHourlySegmentInputSchema>
) {
  const existing = getDatabase()
    .prepare(
      `SELECT id
  ...
  getDatabase()
    .prepare(
      `INSERT INTO screen_time_hourly_segments (
    ...
    .run(
      ...
      segment.startedAt,
      segment.endedAt,
      segment.totalActivitySeconds,
      segment.pickupCount,
      segment.notificationCount,
      segment.firstPickupAt,
      segment.longestActivityStartedAt,
...
export function ingestScreenTimeSync(
  pairing: PairingSessionLike,
  payload: z.infer<typeof screenTimeSyncPayloadSchema>,
  sourceDevice = "iPhone"
) {
  ...
  recordDiagnosticLog({
    level:
      parsed.settings.authorizationStatus === "denied" ||
    ...
    message:
      parsed.settings.authorizationStatus === "approved"
...
export function getScreenTimeSettings(userIds?: string[]) {
  ...
  const capturedDayCount = getDatabase()
    ...
    .get(effectiveUserId) as { count: number };
...
export function updateScreenTimeSettings(
  userId: string,
  patch: z.input<typeof screenTimeSettingsPatchSchema>
) {
  ...
  return upsertScreenTimeSettings(userId, {
    ...
    lastCaptureStartedAt:
      parsed.lastCaptureStartedAt ?? existing.lastCaptureStartedAt,
...
export function getScreenTimeOverlapSummary(input: {
  ...
  const appRowsBySegment = new Map<string, ScreenTimeAppUsageRow[]>();
  ...
  const categoryRowsBySegment = new Map<string, ScreenTimeCategoryUsageRow[]>();
  ...
  const weightedCategories: Array<{ categoryLabel: string; totalActivitySeconds: number }> = [];
  ...
  segmentRows.forEach((row) => {
    ...
    const segmentSeconds = Math.max(
      1,
      overlapSeconds(row.started_at, row.ended_at, row.started_at, row.ended_at)
    );
...
export function getScreenTimeDayDetail(input: { date?: string; userIds?: string[] }) {
  const targetDate =
  ...
  const hourlySegments = segmentRows.map((row) =>
    mapHourlySegment(
      row,
      appRowsBySegment.get(row.id) ?? [],
      categoryRowsBySegment.get(row.id) ?? []
    )
  );
  ...
  const fallbackSummary = {
    totalActivitySeconds: hourlySegments.reduce(
      (sum, segment) => sum + segment.totalActivitySeconds,
      0
    ),
    pickupCount: hourlySegments.reduce((sum, segment) => sum + segment.pickupCount, 0),
    notificationCount: hourlySegments.reduce(
      (sum, segment) => sum + segment.notificationCount,
      0
    ),
  ...
  const topApps = aggregateAppUsage(
    appRows.map((row) => ({
      bundleIdentifier: row.bundle_identifier,
      displayName: row.display_name,
      categoryLabel: row.category_label,
  ...
  const topCategories = aggregateCategoryUsage(
  ...
  return {
    ...
    summary: {
      totalActivitySeconds:
        dayRows.reduce((sum, row) => sum + row.total_activity_seconds, 0) ||
        fallbackSummary.totalActivitySeconds,
      pickupCount:
        dayRows.reduce((sum, row) => sum + row.pickup_count, 0) ||
        fallbackSummary.pickupCount,
      notificationCount:
        dayRows.reduce((sum, row) => sum + row.notification_count, 0) ||
        fallbackSummary.notificationCount,
      firstPickupAt: firstPickupCandidates[0] ?? null,
      longestActivitySeconds:
        dayRows.reduce((max, row) => Math.max(max, row.longest_activity_seconds), 0) ||
        fallbackSummary.longestActivitySeconds,
      activeHourCount: hourlySegments.filter(
        (segment) => segment.totalActivitySeconds > 0
      ).length,
      averageHourlyActivitySeconds: round(
        average(
          hourlySegments
...
export function getScreenTimeMonthSummary(input: { month?: string; userIds?: string[] }) {
  const monthKey =
  ...
  return {
    ...
    totals: {
      totalActivitySeconds: dayRows.reduce(
        (sum, row) => sum + row.total_activity_seconds,
        0
      ),
      pickupCount: dayRows.reduce((sum, row) => sum + row.pickup_count, 0),
      notificationCount: dayRows.reduce(
        (sum, row) => sum + row.notification_count,
        0
      ),
      activeDays: dayRows.filter((row) => row.total_activity_seconds > 0).length
    },
    topApps: aggregateAppUsage(
      appRows.map((row) => ({
        bundleIdentifier: row.bundle_identifier,
        displayName: row.display_name,
        categoryLabel: row.category_label,
    ...
    topCategories: aggregateCategoryUsage(
      categoryRows.map((row) => ({
        categoryLabel: row.category_label,
        totalActivitySeconds: row.total_activity_seconds
      }))
...
export function getScreenTimeAllTimeSummary(userIds?: string[]) {
  ...
  const weekdayMap = new Map<
    number,
    { weekday: number; totalActivitySeconds: number; pickupCount: number; notificationCount: number; days: number }
  >();
  dayRows.forEach((row) => {
  ...
  return {
    summary: {
      ...
      totalNotifications: dayRows.reduce(
        (sum, row) => sum + row.notification_count,
        0
      ),
      averageDailyActivitySeconds: round(
        average(dayRows.map((row) => row.total_activity_seconds))
      ),
      averageDailyPickups: round(
        average(dayRows.map((row) => row.pickup_count)),
        1
      )
    },
    weekdayPattern: [...weekdayMap.values()]
      ...
      .map((row) => ({
        ...
        averageNotifications: round(
          row.notificationCount / Math.max(1, row.days),
          1
        )
      })),
    topApps: aggregateAppUsage(
