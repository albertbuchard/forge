...
function resolveProcessorAgentProfiles(processor: AiProcessor, secrets: SecretsManager) {
  ...
  return requestedAgentIds.map((agentId) => {
    let connection =
      allConnections.find((entry) => entry.agentId === agentId) ?? null;
...
export async function runAiProcessor(
  ...
  const linkedContext = links
    .map(
      (link) =>
        `Linked widget ${link.sourceWidgetId} offers ${link.capabilityMode} access (${link.accessMode}). Metadata: ${JSON.stringify(link.metadata)}`
    )
    .join("\n");
  const fullPrompt = [
  ...
  getDatabase()
    .prepare(
      `UPDATE ai_processors SET last_run_at = ?, last_run_status = ?, updated_at = ? WHERE id = ?`
    )
    ...
    agents.map(async (agent) => {
      ...
      const result = await services.llm.runTextPrompt(
        agent.profile,
        {
          ...
          systemPrompt:
            "You are an AI processor widget inside Forge. Follow the given prompt flow and return the final output only.",
          prompt: fullPrompt
        }
      );
