...
function mapTask(row: TaskRow, time: TaskTimeSummary = emptyTaskTimeSummary()): Task {
...
function normalizeCompletedAt(status: string, existingCompletedAt: string | null): string | null {
  if (status === "done") {
...
function inferReopenStatus(taskId: string): Task["status"] {
  ...
  const metadata = JSON.parse(row.metadata_json) as { previousStatus?: Task["status"] };
  return metadata.previousStatus && metadata.previousStatus !== "done" ? metadata.previousStatus : "focus";
}

function updateTaskRecord(current: Task, input: UpdateTaskInput, activity?: ActivityContext): Task | undefined {
...
function insertTaskRecord(input: CreateTaskInput, activity?: ActivityContext): Task {
...
export function listTasks(filters: TaskListQuery = {}): Task[] {
  ...
  return filterDeletedEntities(
    "task",
    rows.map((row) => mapTask(row, workTime.taskSummaries.get(row.id) ?? emptyTaskTimeSummary()))
  );
}
...
export function createTask(input: CreateTaskInput, activity?: ActivityContext): Task {
...
export function createTaskWithIdempotency(
  ...
  return runInTransaction(() => {
    ...
    const existing = getDatabase()
      ...
      .get(idempotencyKey) as { task_id: string; request_fingerprint: string } | undefined;
...
export function updateTask(taskId: string, input: UpdateTaskInput, activity?: ActivityContext): Task | undefined {
...
export function updateTaskInTransaction(taskId: string, input: UpdateTaskInput, activity?: ActivityContext): Task | undefined {
  const current = getTaskById(taskId);
  if (!current) {
    return undefined;
  }
...
export function uncompleteTask(taskId: string, activity?: ActivityContext): Task | undefined {
  const current = getTaskById(taskId);
  if (!current) {
    return undefined;
  }
...
export function deleteTask(taskId: string, activity?: ActivityContext): Task | undefined {
  const current = getTaskById(taskId);
  if (!current) {
    return undefined;
  }
