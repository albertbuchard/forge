...
function replaceWorkItemGitRefs(
  ...
  const insert = database.prepare(
    `INSERT INTO work_item_git_refs (
...
function assertWorkItemHierarchy(options: {
  ...
  if (options.level === "issue") {
    if (options.parentWorkItemId) {
      throw new HttpError(
        409,
        "issue_parent_not_allowed",
        "Issues must live directly under a project"
      );
    }
    if (!options.projectId) {
      throw new HttpError(
        400,
        "issue_project_required",
  ...
  if (!parent) {
    throw new HttpError(
      ...
      `Parent work item ${options.parentWorkItemId} does not exist`
    );
  }
  if (
    options.projectId &&
    ...
    throw new HttpError(
      ...
      "Parent and child work items must belong to the same project"
    );
  }
  if (options.level === "task" && parent.level !== "issue") {
    throw new HttpError(
      409,
      "task_parent_invalid",
      "Tasks can only live under issues"
    );
  }
  if (options.level === "subtask" && parent.level !== "task") {
    throw new HttpError(
      ...
      "Subtasks can only live under tasks"
    );
  }
}
...
function nextSortOrder(status: string): number {
  const row = getDatabase()
    .prepare(
      `SELECT COALESCE(MAX(sort_order), -1) AS max_sort FROM tasks WHERE status = ?`
    )
    .get(status) as { max_sort: number };
...
function resolveProjectAndGoalIds(
  input: { goalId?: string | null; projectId?: string | null },
  current?: Task
) {
  const currentGoalId =
    current?.goalId && getGoalById(current.goalId) ? current.goalId : null;
  ...
  if (requestedProjectId) {
    ...
    if (!project) {
      throw new HttpError(
        404,
        "project_not_found",
        `Project ${requestedProjectId} does not exist`
      );
    }
    ...
    if (
      ...
      throw new HttpError(
        409,
        "project_goal_mismatch",
        `Project ${requestedProjectId} does not belong to goal ${requestedGoalId}`
      );
    }
    return {
...
function inferReopenStatus(taskId: string): Task["status"] {
  const row = getDatabase()
    .prepare(
...
function updateTaskRecord(
  ...
  getDatabase()
    ...
    .run(
      ...
      input.blockerLinks === undefined
        ? JSON.stringify(current.blockerLinks)
        : JSON.stringify(input.blockerLinks),
      input.completionReport === undefined
        ...
        : input.completionReport === null
          ? null
          : JSON.stringify(input.completionReport),
      completedAt,
      updatedAt,
      nextProjectId,
      current.id
  ...
  if (updated && activity) {
    ...
    const eventType =
      statusChanged && updated.status === "done"
        ? "task_completed"
  ...
  if (updated) {
    createLinkedNotes(
      input.notes,
      { entityType: "task", entityId: updated.id, anchorKey: null },
...
function fingerprintTaskCreate(input: CreateTaskInput): string {
...
function insertTaskRecord(
  ...
  getDatabase()
    ...
    .run(
      ...
      parsed.priority,
      assignment.ownerLabel,
      relationState.goalId,
      relationState.projectId,
      parsed.dueDate,
      parsed.effort,
      parsed.energy,
      parsed.points,
      parsed.plannedDurationSeconds,
      parsed.schedulingRules === null
        ? null
        : JSON.stringify(parsed.schedulingRules),
      sortOrder,
      parsed.status === "done" ? "completed" : null,
      null,
      parsed.level,
      parsed.parentWorkItemId,
      parsed.aiInstructions,
      parsed.executionMode,
      JSON.stringify(parsed.acceptanceCriteria),
      JSON.stringify(parsed.blockerLinks),
      parsed.completionReport === null
        ? null
        : JSON.stringify(parsed.completionReport),
      completedAt,
      now,
      now
    );
...
export function listTasks(filters: TaskListQuery = {}): Task[] {
  ...
  if (filters.tagId) {
    whereClauses.push(
      "EXISTS (SELECT 1 FROM task_tags WHERE task_tags.task_id = tasks.id AND task_tags.tag_id = ?)"
    );
  ...
  if (filters.due === "overdue") {
    whereClauses.push(
      "status != 'done' AND due_date IS NOT NULL AND due_date < ?"
    );
  ...
  if (filters.due === "week") {
    whereClauses.push(
      "status != 'done' AND due_date IS NOT NULL AND due_date >= ? AND due_date <= ?"
    );
...
export function createTaskWithIdempotency(
  input: CreateTaskInput,
  ...
  return runInTransaction(() => {
    ...
    const existing = getDatabase()
      ...
      | { task_id: string; request_fingerprint: string }
      | undefined;
    ...
    if (existing) {
      ...
      if (!task) {
        throw new HttpError(
          ...
          `Stored task ${existing.task_id} for idempotency key is missing`
        );
      }
    ...
    getDatabase()
      .prepare(
        `INSERT INTO task_create_idempotency (idempotency_key, request_fingerprint, task_id, created_at)
         VALUES (?, ?, ?, ?)`
      )
...
export function updateTaskInTransaction(
  taskId: string,
  input: UpdateTaskInput,
  activity?: ActivityContext
): Task | undefined {
...
export function uncompleteTask(
  ...
  return runInTransaction(() =>
    updateTaskRecord(current, { status: inferReopenStatus(taskId) }, activity)
  );
}
