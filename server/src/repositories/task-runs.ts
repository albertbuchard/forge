...
function readExecutionConfig(): ExecutionConfig {
  try {
    const row = getDatabase()
      .prepare(
        `SELECT max_active_tasks, time_accounting_mode
         FROM app_settings
         WHERE id = 1`
      )
...
function getTaskRunRowById(taskRunId: string): TaskRunRow | undefined {
  return getDatabase()
    .prepare(
      `${selectClause()}
       WHERE task_runs.id = ?`
...
function getActiveTaskRunRow(taskId: string, now: Date): TaskRunRow | undefined {
  return getDatabase()
    ...
    .get(taskId, now.toISOString()) as TaskRunRow | undefined;
}
...
function buildTaskRunErrorDetails(
  run: TaskRunRow | undefined,
  now: Date,
  details: Omit<TaskRunErrorDetails, "taskRun"> = {}
): TaskRunErrorDetails {
  const cached = computeWorkTime(now);
  const response: TaskRunErrorDetails = {
    ...details,
...
function assertActorMatch(run: TaskRunRow, actualActor: string | undefined, now: Date): void {
  if (actualActor && actualActor !== run.actor) {
    throw new HttpError(
      409,
      "task_run_actor_conflict",
      `Task run is owned by ${run.actor}, not ${actualActor}`,
      buildTaskRunErrorDetails(run, now, { requestedActor: actualActor })
    );
  }
}

function requireKnownTask(taskId: string): void {
  if (!getTaskById(taskId)) {
    throw new HttpError(404, "task_not_found", `Task ${taskId} does not exist`);
  }
}

function setCurrentRunInTransaction(actor: string, taskRunId: string): void {
  ...
  db.prepare(
    `UPDATE task_runs SET is_current = 0 WHERE actor = ? AND status = 'active' AND is_current = 1`
  ).run(actor);
  db.prepare(
    `UPDATE task_runs SET is_current = 1 WHERE id = ? AND actor = ? AND status = 'active'`
  ).run(taskRunId, actor);
}

function ensureCurrentRunExistsInTransaction(actor: string, now: Date): void {
  const current = getDatabase()
    ...
    .get(actor, now.toISOString()) as { id: string } | undefined;
  if (current) {
    return;
  }
...
function enforceActiveRunLimit(actor: string, taskId: string, now: Date): void {
  ...
  throw new HttpError(
    ...
    `Cannot start ${taskId} because ${actor} already has ${activeRuns.length} active task timers (limit ${config.maxActiveTasks}).`,
    {
      activeRuns: activeRuns.map((run) => mapTaskRun(run, now)),
      limit: config.maxActiveTasks,
      timeAccountingMode: config.timeAccountingMode
    } satisfies TaskRunErrorDetails
  );
}
...
function promoteFallbackCurrentRun(actor: string, now: Date): void {
  ensureCurrentRunExistsInTransaction(actor, now);
}
...
export function claimTaskRun(
  taskId: string,
  input: TaskRunClaimInput,
  now = new Date(),
  activity: ActivityContext = { source: "ui" }
): { run: TaskRun; replayed: boolean } {
  return runInTransaction(() => {
    ...
    if (scheduling.blocked && (!parsedInput.overrideReason || parsedInput.overrideReason.trim().length === 0)) {
      throw new HttpError(
        ...
        `Calendar rules block starting ${task.title} right now. Add an override reason to proceed.`,
        {
          taskId,
    ...
    if (existing) {
      ...
      getDatabase()
        ...
        .run(
          parsedInput.timerMode,
          parsedInput.plannedDurationSeconds,
          existing.is_current,
          nowIso,
          nextExpiry,
  ...
  return runInTransaction(() => {
    ...
    if (current.status !== "active") {
      throw new HttpError(
        409,
        "task_run_not_active",
        `Task run ${taskRunId} is ${current.status} and cannot accept heartbeats`,
  ...
  return runInTransaction(() => {
    ...
    if (current.status !== "active") {
      throw new HttpError(
        409,
        "task_run_not_active",
        `Task run ${taskRunId} is ${current.status} and cannot be focused`,
        buildTaskRunErrorDetails(current, now)
      );
    }
    assertActorMatch(current, input.actor, now);
