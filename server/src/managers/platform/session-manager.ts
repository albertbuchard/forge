...
export class SessionManager extends AbstractAuditedManager {
  ...
  ensureLocalOperatorSession(headers: Record<string, unknown>, reply: FastifyReply): OperatorSession {
    ...
    this.databaseManager
      ...
      .run(
        sessionId,
        this.secretsManager.hashSecret(sessionToken),
        actorLabel,
  ...
  readSessionFromHeaders(headers: Record<string, unknown>): OperatorSession | null {
    ...
    const sessionCookie = pairs.find((entry: string) => entry.startsWith(needle));
    if (!sessionCookie) {
      return null;
    }
    ...
    const row = this.databaseManager
      ...
      .get(this.secretsManager.hashSecret(sessionToken)) as OperatorSessionRow | undefined;
  ...
  private isLocalOperatorRequest(headers: Record<string, unknown>) {
