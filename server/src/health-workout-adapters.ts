...
export type WorkoutActivityDescriptor = z.infer<
  typeof workoutActivityDescriptorSchema
>;
export type WorkoutDetails = z.infer<typeof workoutDetailsSchema>;

type WorkoutSourceAdapter = {
  sourceSystem: string;
  normalizeActivity(input: {
    workoutType: string;
    existingActivity?: WorkoutActivityDescriptor | null;
  }): WorkoutActivityDescriptor;
};

type WorkoutPresentationSeed = {
  source: string;
  sourceType: string;
  workoutType: string;
...
type WorkoutActivityCatalogEntry = {
  key: string;
  label: string;
};

const APPLE_HEALTH_ACTIVITY_TYPES = new Map<number, WorkoutActivityCatalogEntry>([
...
function resolveActivityFamily(key: string) {
...
function buildFallbackActivity(
  sourceSystem: string,
  workoutType: string,
  providerActivityType = "generic_workout_type",
  providerRawValue: number | null = null,
  isFallback = false
): WorkoutActivityDescriptor {
  const canonicalKey = normalizeCanonicalKey(
    workoutType.length > 0 ? workoutType : "workout"
  );
...
function normalizeAppleHealthActivity(
  workoutType: string,
  existingActivity?: WorkoutActivityDescriptor | null
): WorkoutActivityDescriptor {
...
const workoutSourceAdapters = new Map<string, WorkoutSourceAdapter>([
  [
    "apple_health",
    {
      sourceSystem: "apple_health",
      normalizeActivity: ({ workoutType, existingActivity }) =>
        normalizeAppleHealthActivity(workoutType, existingActivity)
    }
  ],
  [
    "forge",
...
function getWorkoutSourceAdapter(sourceSystem: string) {
  return (
    workoutSourceAdapters.get(sourceSystem) ??
    ({
      sourceSystem,
      normalizeActivity: ({ workoutType, existingActivity }) => {
        if (existingActivity) {
    ...
    } satisfies WorkoutSourceAdapter)
  );
}

function normalizeWorkoutDetails(
  sourceSystem: string,
  value: unknown
): WorkoutDetails {
  ...
  return {
    ...
    events: [...parsed.data.events].sort((left, right) =>
      left.startedAt.localeCompare(right.startedAt)
    ),
    components: [...parsed.data.components].sort((left, right) =>
      left.startedAt.localeCompare(right.startedAt)
    )
  };
}

export function buildWorkoutSessionPresentation(input: WorkoutPresentationSeed) {
  const provenance = input.provenance ?? {};
  const derived = input.derived ?? {};
  const sourceSystem = inferSourceSystem(input.source, input.sourceType, provenance);
  const storedActivity =
    workoutActivityDescriptorSchema.safeParse(derived.activity).success
...
export function buildWorkoutSessionPersistenceSeed(input: WorkoutPersistenceSeed) {
  const sourceSystem =
    cleanString(input.sourceSystem) ??
