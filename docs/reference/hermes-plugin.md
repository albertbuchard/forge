...
- permanent Work uses `forge_call_work_route` for Work Engagements, confirmed check-ins and trends, Opportunity Campaigns, sourced opportunities, criteria-versioned evaluations, applications, supporting records, relationships, search runs, accepted offers, and reviewed external transmission; it is separate from task execution and private import remains local-operator-only
